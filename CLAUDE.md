...
> "系统能支撑 50+ 章长篇网文的稳定生成，人类可随时深度介入创作，跨章一致性可自动检测，风格可多样化配置。"

V2.0 验证范围：scifi（科幻新怪谈）+ webnovel_intense（网文高刺激模式）为主验证路径，xuanhuan/urban/literary 为兼容性验证。
...
21. ReviewMerger 只做内存合并，**不调用 LLM**，耗时须 < 10ms
...
- 拆书分析
- 完整 Studio
- 风格迁移（V2.0 部分实现 → Task 032）
- 角色心理模型（V2.5+）
- 读者情绪模拟（V2.5+）
- PolyphonyPlanner（V2.5+）
- CharacterAutonomyAuditor（V2.5+）
- ~~ForeshadowingManager（V2.0 → Task 030 ContinuityAuditor）~~
- ~~LongFormContinuityAuditor（V2.0 → Task 030）~~
- MacroNarrativePlanner（V2.5+）
- React Web UI / TUI（V3.0+）
- PostgreSQL / Qdrant / Redis（V3.0+）
- 多模型路由（V3.0+）
...
├── prompts/                      # Agent Prompt 模板 + 工艺卡
│   ├── archive/                  # 历史 Prompt 版本
│   └── cards/                    # YAML 工艺卡（_manifest.yaml + vX.Y.Z.yaml）
├── docs/                         # 项目文档
│   ├── INDEX.md                  # 文档索引
│   ├── STATUS.md                 # 状态看板
...
│   │   ├── summary_writer.py       # 轻量摘要生成
│   │   └── continuity_auditor.py   # V2.0 跨章一致性（Task 030）
│   ├── workflows/                # LangGraph 工作流编排
│   │   ├── phase1_graph.py       # StateGraph 状态机 + 公共 API
│   │   ├── review_merger.py      # Rule + LLM 结果轻量合并
│   │   ├── _nodes.py             # 12 个节点函数
│   │   └── _helpers.py           # 数据加载辅助函数
│   ├── llm/                      # LLM Client + 重试 + JSON 解析
│   ├── prompts/                  # PromptLoader 单例 + 工艺卡加载
│   ├── utils/                    # 质量检测工具（纯代码）
│   ├── genres/                   # Genre Profile 加载器
│   └── creative_modes/           # CreativeModeProfile 注册表
├── tests/                        # pytest + pytest-asyncio
└── evals/                        # 评测集 runner + 种子项目配置
```
