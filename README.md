...
**V5.2 已完成**：默认 `gate_mode` 已切换为 `enforce`，Ch1–Ch150 enforce 模式验证 150/150 accept，长期事实库的可信度达到 V5.2 出口标准。当前准备进入 V6/V7：叙事骨架 MVP、长篇质量度量、可靠长跑底盘。
...
| 当前阶段 | V5.2 已完成：enforce 默认启用 + 150 章完整证据 | CLI 默认 `enforce`，Ch1–Ch150 全部 accept，角色状态、世界设定和数字读数均有正文证据支撑 |

### 当前开发重点

1. **叙事骨架 MVP**：从单章目标上升到卷/弧级叙事结构，为 200+ 章连贯性提供自顶向下约束。
2. **长篇质量度量**：建立跨章质量指标，量化设定回收、角色一致性、伏笔追踪和正文可读性。
3. **可靠长跑底盘**：在 enforce 模式基础上继续验证 200 章以上稳定输出，完善断点续跑与异常恢复。
