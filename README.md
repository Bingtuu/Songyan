...
**V7 阶段 W 已完成 Task 165 真实复跑但尚未出口**：对 V6 生成的 150 章正文人工通读发现，句子级度量掩盖了篇章级质量债（元标记泄漏、整段落重复、时间线矛盾、概念通胀）。Task 160-165 已取得修复后 Ch1–Ch150 证据（150/150 accepted，P 洁净与 L 文学通过），但阶段 W 仍因 T5/T6c harness 不回退项条件不通过；下一步先处理 T5/T6b/T6c 口径，再决定 T9/T10 正式冻结。V7 构想见 [`docs/v7-vision.md`](docs/v7-vision.md)，阶段规划见 [`docs/v7-plan.md`](docs/v7-plan.md)。
...
| 当前阶段（V7） | 先修复 V6 暴露的篇章级质量债（元标记 / 重复 / 时间线 / 概念通胀），再让系统自驱把质量维持在高位并爬坡至 Ch300 | 正文更洁净、连贯；世界观概念持续落地；目标从 200 章推进到 300 章级别稳定输出 |

### 当前开发重点（V7）

1. **篇章级质量修复（阶段 W）**：以确定性工程修复根治 V6 暴露的可读性/连贯性债——元标记泄漏清洗、整段落去重、跨章时间线一致性检测、概念预算约束，并把文本洁净度做成可入库、可查的指标。🔄 Task 165 复跑已完成且 P/L 通过；当前阻断项是 T5/T6c harness 口径处理，完成前不进入 X/Y/Z。
2. **叙事自驱（阶段 X）**：把静态叙事骨架升级为 plan→generate→re-plan 闭环，并让长程伏笔从被动遗忘转为主动兑现调度。◻ 待阶段 W 出口后启动。
3. **enforce 可生产化（阶段 Y）**：把质量门禁从绝对阈值改为相对趋势/异常因子触发，正常波动不误伤、真退化才暂停。◻ 待阶段 W 出口后启动。
4. **Ch300 渐进爬坡（阶段 Z）**：Ch150 基线 → Ch200 → Ch250 → Ch300，每级取真实证据再进下一级。◻ 待 W+X+Y 落地后启动。
...
| 篇章级质量与自驱 | 修复篇章级质量债，再让系统自驱把质量维持在高位并爬坡至 Ch300 | 元标记清洗 / 段落去重 / 时间线一致 / 概念预算 / 文本洁净度 T9 harness（阶段 W）、plan→re-plan 闭环 + 伏笔调度（阶段 X）、enforce 门禁自适应（阶段 Y）、Ch200→300 渐进验证（阶段 Z） | 进行中（V7：Task 165 已完成 Ch1–Ch150 复跑且 P/L 通过；阶段 W 因 T5/T6c harness 不回退项尚未出口，X/Y/Z 待 W 出口后展开） |
...
Songyan 已完成 V6：系统装上了自顶向下的叙事骨架，长篇质量从"无对象、无指标"变成"可见、可度量"，并在治理管线上取得真实 Ch1–Ch150 证据（`run-bba292da`，150/150 accept）。当前进入 V7：先修复 V6 生成的正文中"读者一眼可见"的篇章级质量债，再让系统自驱把质量维持在高位、渐进爬坡到 Ch300。

V7 分四个阶段推进：

1. **篇章级质量修复（阶段 W）**：以确定性工程修复根治元标记泄漏、整段落重复、跨章时间线矛盾和概念通胀，并把文本洁净度做成可入库、可查的指标。Task 165 已完成修复后 Ch1–Ch150 复跑，P 洁净与 L 文学通过；当前闸门卡在 T5/T6c harness 不回退项，处理前不进入阶段 X/Y/Z。
2. **叙事自驱（阶段 X）**：把静态叙事骨架升级为 plan→generate→re-plan 闭环，让长程伏笔从被动遗忘转为主动兑现调度。◻ 待阶段 W 出口后启动。
3. **enforce 可生产化（阶段 Y）**：把质量门禁从绝对阈值改为相对趋势/异常因子触发，正常波动不误伤、真退化才暂停。◻ 待阶段 W 出口后启动。
4. **Ch300 渐进爬坡（阶段 Z）**：Ch150 基线 → Ch200 → Ch250 → Ch300，每级取真实证据再进下一级，每级预留撞墙定点修复占位。◻ 待 W+X+Y 落地后启动。

README 不维护实时任务状态。当前进度、测试结果和下一步执行项请查看 [`docs/STATUS.md`](docs/STATUS.md)；V7 任务事实入口见 [`tasks/V7-README.md`](tasks/V7-README.md)。
