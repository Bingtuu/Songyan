...
| 当前阶段（V6） | 补齐自顶向下叙事架构与长篇质量度量，验证 100-150 章稳定输出 | 章节目标从全书规划派生；orphan、文学质量、伏笔兑现可观测；无人值守 Ch100 长跑已取得证据 |

### 当前开发重点（V6）

1. **叙事骨架 MVP**：从单章目标上升到卷/弧级叙事结构，为 100-150 章连贯性提供自顶向下约束。✅ 已完成。
2. **长篇质量度量**：建立跨章质量指标，量化设定回收、角色一致性、伏笔追踪和正文可读性。✅ 已完成。
3. **可靠长跑底盘**：在 enforce 模式基础上验证 Ch100-150 稳定输出，完善 run 级断点续跑与异常恢复。Ch1-Ch100 已跑通，Ch150 复现与 T5 阈值复核进行中。
...
| 叙事骨架与度量 | 补齐自顶向下叙事架构，并让长篇质量可度量 | 全书大纲 / 弧规划 / 线索经济 MVP（阶段 0）、跨章质量度量入库 orphan/T7/质量债/文学趋势/伏笔兑现率（阶段 A）、末端治理降低 critical 误判与 orphan 累积（阶段 B）、无人值守长跑底盘 run 级 resume / LLM 限流预算 / 失败隔离 / DB 维护（阶段 C）、Ch1-Ch100 长窗口实跑（阶段 D） | 进行中（V6：阶段 0+A+B+C 已完成；**阶段 D 已启动：157 Ch1-Ch50 完成，158 Ch1-Ch100 完成，159 Ch150 复现待执行**） |
...
5. **长窗口验证（阶段 D）**：在 V5.2 + 骨架 + 末端治理 + 长跑底盘合入后，取得 Ch1–Ch150 连续运行证据。**157 Ch1–Ch50 实跑已完成（48/50 accept），158 Ch1–Ch100 长跑已完成（98/100 accept）；159 Ch150 复现 + V6 阶段验收待执行**。◻ 部分启动。
