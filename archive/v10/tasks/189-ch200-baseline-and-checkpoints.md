...
> **状态**: ✅ 完成（DONE：`archive/v10/tasks/189-ch200-baseline-and-checkpoints-DONE.md`）
...
注意：上述命令只用于采集 sci-fi 自身指标。当前 `five_gate_check.py` 默认 baseline 是包内 `scifi_ch100_baseline.json`，对 Ch125+ 会回退到最后一个 Ch100 点。因此，执行者必须从这些输出中提取 metrics，生成受版本管理的 `archive/v10/artifacts/189-scifi-ch200-baseline.json`，供后续非 sci-fi Ch200 显式传入。不得把默认 baseline 输出误当作 Ch200 对照值。

生成 `archive/v10/artifacts/189-scifi-ch200-baseline.json` 后，再用正式工具验证 baseline 文件可被读取：

```powershell
python scripts/five_gate_check.py --genre scifi --db <scifi_ch200.db> --project-id <project_id> --up-to 125 --baseline archive/v10/artifacts/189-scifi-ch200-baseline.json --format json
python scripts/five_gate_check.py --genre scifi --db <scifi_ch200.db> --project-id <project_id> --up-to 150 --baseline archive/v10/artifacts/189-scifi-ch200-baseline.json --format json
python scripts/five_gate_check.py --genre scifi --db <scifi_ch200.db> --project-id <project_id> --up-to 175 --baseline archive/v10/artifacts/189-scifi-ch200-baseline.json --format json
python scripts/five_gate_check.py --genre scifi --db <scifi_ch200.db> --project-id <project_id> --up-to 200 --baseline archive/v10/artifacts/189-scifi-ch200-baseline.json --format json
...
| 文件 | 内容 |
|------|------|
| `archive/v10/artifacts/189-scifi-ch200-baseline.json` | 受版本管理的 canonical baseline，含 Ch125/150/175/200 的 accepted、budget、CED、overdue、health、gap、halt |
...
| `archive/v10/tasks/189-ch200-baseline-and-checkpoints-DONE.md` | 结论、命令、路径、对照表、是否入包建议 |
...
2. Ch125 / Ch150 / Ch175 / Ch200 四个 checkpoint 均有正式工具输出，并已生成 `archive/v10/artifacts/189-scifi-ch200-baseline.json`。
...
6. 后续所有非 sci-fi Ch200 five-gate 命令必须显式传入 `--baseline archive/v10/artifacts/189-scifi-ch200-baseline.json`；文档中不得使用默认 Ch100 baseline 跑 Ch125+。
...
| 误用包内 Ch100 baseline 跑 Ch125+ | 立即作废该输出；重新生成并显式传入 `archive/v10/artifacts/189-scifi-ch200-baseline.json` |
