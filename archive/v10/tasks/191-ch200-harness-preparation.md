...
> **优先级**: P0（192-194 实跑前置）
> **状态**: ✅ 完成（DONE：`archive/v10/tasks/191-ch200-harness-preparation-DONE.md`）
...
python scripts/<ch200_harness>.py --audit --genre <genre> --up-to 150 --baseline archive/v10/artifacts/189-scifi-ch200-baseline.json
python scripts/<ch200_harness>.py --dry-run --genre <genre>
```

`--audit` 对非 sci-fi Ch125+ checkpoint 必须显式传入 `archive/v10/artifacts/189-scifi-ch200-baseline.json`，不得依赖 `scripts/five_gate_check.py` 的默认 baseline（默认值仍是包内 Ch100 baseline，不能作为 Ch200 对照口径）。
...
| source inventory | `.tmp/190_ch100_source_inventory.json` 只作本地工作副本；canonical 事实源为 `archive/v10/tasks/190-ch100-terminal-source-inventory-DONE.md`。若 `.tmp` inventory 缺失，dry-run 必须提示重建本地副本或要求显式传入 source 参数，不得静默读取默认库或猜测路径。 |
...
13. 产出 `archive/v10/tasks/191-ch200-harness-preparation-DONE.md`，并更新 `tasks/V10-README.md`。
...
| `.tmp` source inventory 在 fresh checkout 缺失 | 以 `archive/v10/tasks/190-ch100-terminal-source-inventory-DONE.md` 为 canonical；dry-run 提示重建本地 `.tmp` 副本或要求显式 source 参数 |
| baseline 文件格式未冻结 | 依赖 Task 189；audit 命令必须显式传入 `archive/v10/artifacts/189-scifi-ch200-baseline.json` |
