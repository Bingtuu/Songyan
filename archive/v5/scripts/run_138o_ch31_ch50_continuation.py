...
说明:
    - 延续 Task 138n 的验证项目，从 Ch31 跑到 Ch50，不克隆新项目。
    - 临时切换 Writer default_version 为 1.2.0，退出时恢复。
    - 默认使用 gate_mode="observe"，记录但不主动因 health_low 暂停。
    - 每章输出关键指标；每 10 章输出趋势摘要。
    - 生成 docs/reports/task-138o-ch31-ch50-long-window-validation-report.md。
"""
...
PROJECT_ID = os.getenv("PROJECT_ID", "987fccbd53414e3da76da0fe07f887a9")
START_CHAPTER = int(os.getenv("START_CHAPTER", "31"))
...
REPORT_PATH = Path("docs/reports/task-138o-ch31-ch50-long-window-validation-report.md")
METRICS_PATH = Path(".tmp/task138o_ch31_ch50_metrics.jsonl")
...
def _write_report(
    ...
    if halt_reason:
        lines.append(
            f"实跑触发 halt：{halt_reason}。需根据根因决定是否另起 Task 138p 处理。"
        )
    elif len(completed) == target_count:
        lines.append(
            f"Ch{START_CHAPTER}-Ch{END_CHAPTER} 全部完成，无 AutoHalt。"
            "请结合 continuity 趋势判断 138n 改进在长窗口下是否稳定。"
        )
    else:
        lines.append(
...
async def main() -> None:
    ...
    if db_path.name == "songyan.db":
        raise RuntimeError(
            "Refusing to run continuation in main DB. "
            "Use .tmp/task138n_ch1_ch30_rerun.db and set DATABASE_URL."
        )

    project = await ProjectRepository().get(PROJECT_ID)
    if project is None:
        raise ValueError(f"Continuation project not found: {PROJECT_ID}")
    project_id = PROJECT_ID
    print(f"[project] Continuing rehearsal project: {project_id}")
    print(f"[preflight] db={db_path}")
    print(f"[preflight] chapter_range=({START_CHAPTER}, {END_CHAPTER})")
