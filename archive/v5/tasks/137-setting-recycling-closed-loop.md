...
---
> **V5.2 收口说明**：本任务保持活跃期间，工作已由 Task 138a–138f 承接完成；Task 138e 明确 Task 137 不归档，当前状态为「已关闭」。
...
|---|---|---|---|
| 127 | `archive/v5/tasks/127-health-low-score-halt-refactor-DONE.md` | 已完成。`health_low_score_halt` 改为“历史新低 + P1 同步激增”复合条件，Ch1-Ch19 enforce 小窗口零 gate 触发，pytest 1842 passed。 | health_low 硬门禁不再是当前主阻塞，后续验证可聚焦 orphan/continuity。 |
| 128 | `archive/v5/tasks/128-strict-mode-fault-tolerance-and-quality-ramp-DONE.md` | 已完成。QG false 在开局期可走 `degraded_accept`，RevisionHandler 增加 readability 专项路径，pytest/ruff 通过。 | enforce 流程容错已具备，但 degraded_accept 章节仍需在验证口径中单独统计 settlement/summary。 |
| 129 | `archive/v5/tasks/129-enforce-mode-ch1-ch50-validation-DONE.md` | 条件完成。`run-89d7a2d4` Ch1-Ch15 后因 quality gate streak 暂停，暴露 Writer 单场景退化、SettlementExtractor 提取失败、orphan 快速累积。 | 明确 V5.2 底层缺陷来源；Task 133/134/135 已分别承接前三类缺陷。 |
| 130 | `archive/v5/tasks/130-gate-mode-default-decision-DONE.md` | 已完成。默认 `gate_mode` 保持 `observe`，CLI 暴露 `--gate-mode`，报告支持 gate 汇总。 | 当前不应默认切换 enforce；Task 137 仍使用显式验证窗口收集证据。 |
| 131 | `archive/v5/tasks/131-task-docs-archive-and-status-cleanup-DONE.md` | 已完成。历史规划稿归档，索引优先指向 `-DONE.md`。 | 后续任务必须保持活跃规划稿与 DONE 文档边界清晰。 |
| 132 | `archive/v5/tasks/132-v51-final-acceptance-package-DONE.md` | 已完成。V5.1 通过，Task 129 暴露的底层缺陷转入 V5.2。 | Task 137 属于 V5.2 收口，不影响 V5.1 通过结论。 |
| 133 | `archive/v5/tasks/133-writer-multi-scene-structure-fix-DONE.md` | 已完成。Writer 1.2.0 多场景结构在 Task 136 采集窗口中达到 100%。 | 多场景结构不是当前阻塞；Writer 1.2.0 仍需显式启用，默认仍为 1.1.0。 |
| 134 | `archive/v5/tasks/134-settlement-character-numerical-extraction-fix-DONE.md` | 已完成。SettlementExtractor 角色/数值提取旧口径在成功 settlement 章节中达标。 | 后续复跑必须同时统计全窗口 settlement+summary，不能只看成功 settlement 子集。 |
| 135 | `archive/v5/tasks/135-setting-recycling-and-continuity-health-governance-DONE.md` | 已完成。health floor 达标，但 orphan 增速问题未完全解决。 | Task 137 的直接前置：继续治理 orphan 虚假累积与 tracking 刷新。 |
| 136 | `archive/v5/tasks/136-v52-enforce-ch1-ch20-validation-DONE.md` | 已完成但整体验收未通过。Ch1-Ch20 完成率、多场景、health floor 通过；orphan 增长速率未减半，Ch15/Ch16 以 degraded_accept 完成。 | 触发 Task 137；当前不能扩大默认 enforce 验证，需先修 orphan 收口。 |
...
1. `archive/v5/tasks/138a-remaining-orphan-classification-DONE.md`：分类最新剩余 16 个 orphan。
2. `archive/v5/tasks/138b-orphan-root-cause-decision-DONE.md`：基于分类结果决定最小动作。
3. `archive/v5/tasks/138c-orphan-minimal-fix-DONE.md`：实施最小修复或 human mark。
4. `archive/v5/tasks/138d-ch10-ch12-post-fix-rerun-DONE.md`：用副本 DB 复跑 Ch10-Ch12。
5. `archive/v5/tasks/138e-task137-fact-sync-and-closure-DONE.md`：同步事实源并决定 Task 137 归档或进入下一轮。
...
- 报告: `archive/v5/reports/task-137-v52-enforce-ch1-ch20-rerun-report.md`
...
- 报告: `archive/v5/reports/task-137-ch10-focus-validation-report.md`
- 结论:
  - 清理策略有效，未污染主库。
...
- 同步对象: `.trae/specs/complete-v51-remaining-tasks/tasks.md`、`checklist.md`、`progress.md`、`docs/STATUS.md`、`tasks/V5-README.md`、`README.md`、`docs/INDEX.md`、`archive/v5/tasks/138e-task137-fact-sync-and-closure-DONE.md`、`archive/v5/reports/task-137-ch10-focus-validation-report.md`。
...
- 新增/补强测试文件
- `archive/v5/reports/task-137-v52-enforce-ch1-ch20-rerun-report.md`（实跑后生成，结果未通过）
