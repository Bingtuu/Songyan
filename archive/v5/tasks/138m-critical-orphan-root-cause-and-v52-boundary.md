...
>
> **完成结论**: 本任务已按执行计划完成。决策报告见 `archive/v5/reports/task-138m-critical-orphan-root-cause-report.md`。根因结论：35 个 P1 orphan 中 27 个为 Ch20+ 新引入后立即丢弃的 critical 设定；26 个因 `MAX_ORPHANED=8` 等约束预算截断从未获得 human_mark；全部 35 个均进入无上限的 `mandatory_references`（Ch30 达 43 条）但 Writer 未执行。推荐方案为 **A + C**（QG 阻断式 revision + mandatory_reference 上限/衰减调优），后续任务见 `archive/v5/tasks/138n-qg-mandatory-reference-revision-loop-DONE.md`。
>
...
4. **形成决策文档**：输出 `archive/v5/reports/task-138m-critical-orphan-root-cause-report.md`，明确推荐方案、风险、所需工作量。
...
`archive/v5/reports/task-138m-critical-orphan-root-cause-report.md` 必须包含：
...
- [x] 决策报告已输出到 `archive/v5/reports/task-138m-critical-orphan-root-cause-report.md`。
...
- 138k 报告：`archive/v5/reports/task-138k-long-window-rehearsal-report.md`
- 138k 任务：`archive/v5/tasks/138k-long-window-rehearsal-ch1-ch50.md`
- 138h-138j DONE：`archive/v5/tasks/138h-critical-orphan-mandatory-recall-loop-DONE.md`
- 138l DONE：`archive/v5/tasks/138l-settlement-telemetry-false-positive-fix-DONE.md`
