...
>
> **Ch13 根因**: 不是 settlement 失败或 MR 缺失，而是 3 轮 revision 后 quality gate 仍无法收敛，structural rewrite（v-13-4）因缺少结尾钩子失败，系统回滚到 rev-13-3 做降级接受（`quality_gate_passed=False`）。该章节 `mandatory_reference_issue_count=0`，与 A+C 改动无直接因果关系，判定为随机波动。报告见 `archive/v5/reports/task-138n-ch1-ch30-rerun-report.md`。
>
...
- Task 138m 决策报告：`archive/v5/reports/task-138m-critical-orphan-root-cause-report.md`
- Task 138k 长窗口报告：`archive/v5/reports/task-138k-long-window-rehearsal-report.md`
- Task 138h 强制回收闭环：`archive/v5/tasks/138h-critical-orphan-mandatory-recall-loop-DONE.md`
- 关键代码：
