...
3. **173 挂起项**：回归实跑收尾时观察进程 ≤60s 自然退出 ×2（第一次 end10、第二次 resume 续跑收尾），结果补录 `archive/v9/173-interpreter-exit-hang-fix-DONE.md` 后状态翻 ✅。
4. **174 挂起项**：用该 run 做一次三边重建演示（应用日志按 run_id+chapter 过滤 ↔ chapter_runs JSONL ↔ DB chapter_versions/settlements 三处交叉一致），证据补录 `archive/v9/174-logging-system-foundation-DONE.md` 后状态翻 ✅。
