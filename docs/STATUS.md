...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2234 passed, 2 skipped, 1 xfailed, 2 warnings`（Task 158 完成后）；`ruff check src/ tests/ scripts/run_158_ch1_ch100.py` 通过 |
...
| 下一步规划 | **V6 阶段 C（工程加固 Task 153-156）已完成**；**阶段 D 已启动**：157a V6 验收判据 harness 已交付并配套 32 个 Layer 2 单测，全量 pytest `2234 passed, 2 skipped, 1 xfailed`，`ruff check` 通过；157b Ch1-Ch50 无人值守实跑已执行；**158 Ch1-Ch100 长跑已完成**（run `run-10d7961b`，100/100 完成，98/100 accept，报告 `docs/reports/task-158-ch1-ch100-long-run-validation-report.md`），T5 首次实测未冻结（Ch50/Ch70 扫描耗时破线，待 159 复核），kill→resume 真实演练未形成命令级证据。后续 **159 Ch150 复现并产出 V6 验收报告**；事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md` 与 `docs/v7-vision.md`。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **V6 阶段 D 进行中**：**Task 158 已完成**——Ch1-Ch100 无人值守长跑（run `run-10d7961b`，隔离 DB `.tmp/task158_ch1_ch100.db`，enforce + isolate，真实 DeepSeek API，≈7.8h）。**结果：100/100 完成，98/100 accept，无 AutoHalt/门禁触发**；harness 判定 T1/T6a/T4/health≥7.0 通过；**T2=98/100**（Ch48、Ch62 settlement/summary 失败被 isolate）；**T3/T8** 文学趋势在 Ch10 起触线（literary/conceptual/fissure）；**T5** DB 尺寸 84.78MB 未破线，但扫描耗时在 Ch50/Ch70 达 1.93×/1.76× 基线而破线，按 148z 纪律需 Task 159 复核后再冻结；**T6c** 因新 critical 速率被骨架压到 0，归因比值判据在退化基线下算术失真（与 157 同因）。报告 `docs/reports/task-158-ch1-ch100-long-run-validation-report.md`，DONE 文档 `tasks/158-ch1-ch100-long-run-validation-DONE.md`。**诚实记录**：§1.3-R 要求的"人为 kill 后同命令 `--resume` 续完"尚未取得真实命令级证据（脚本 `--kill-at-chapter` 调试钩子未启用；Ch11 曾内部 settlement 失败重算，但属 run 内恢复）。**下一步：Task 159 Ch150 复现 + V6 阶段验收**；建议 159 启动前补一次 `--kill-at-chapter` 真实 kill→resume 演练，并把 T5 阈值复核纳入 159 报告。
