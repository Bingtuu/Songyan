...
1. **V10.2 任务书准备（下一步）**：基于 Task 190 起点判定和 Task 191 harness，撰写 192-194 的正式执行任务书。
2. **xuanhuan Ch100 重建**：DB 已覆盖不可用，需重新跑 Ch100（REBUILD_REQUIRED），再进入 Ch200 爬坡。
3. **wuxia Ch28 clean**：T9=1（Ch28 省略号占位段），需在 Task 193 启动前执行 deterministic clean 并重跑 T9 确认 T9=0；其余章节可直接续跑。
4. **urban Ch200 准入**：urban 是当前唯一 `CONTINUE_READY` 体裁，可用 `scripts/run_v10_ch200_climb.py --init-from-source --genre urban` 初始化 V10 Ch200 DB，真实长跑仍需单独任务书批准。
