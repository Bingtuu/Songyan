...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V6 进行中**：阶段 0（叙事骨架 MVP，Task 141-144）+ 阶段 A（长篇质量度量，Task 145-148 + 148z）**已完成**；阶段 B/C/D（末端治理 / 工程加固 / 长窗口验证）待做。Task 141-159，事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md`，论证基础 `docs/300-chapter-gap-analysis.md`。前置 V5.2 已完成验收（下行）。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2099 passed, 2 skipped, 1 xfailed, 2 warnings`（V6 阶段 A 完成后）；`ruff check src/ tests/` 通过 |
...
| Python | 3.11.9 |
| 后续阶段 | **下一步 V6 阶段 B（末端治理，Task 149-152）**——须在骨架+度量落地后开工，现已满足；随后阶段 C（工程加固 153-156）、阶段 D（长窗口验证 157-159）。事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md` |
...
1. **V6 阶段 0 + A 启动**：Task 141-144 叙事骨架 MVP 与 Task 145-148 度量同步同期推进，事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md`。**Task 141（叙事骨架数据模型）已完成**（`tasks/141-narrative-skeleton-data-model-DONE.md`）：新增 `StoryOutline`/`ArcPlan`/`PlotThread` 模型 + 三张表（story_outlines/arc_plans/plot_threads）+ `NarrativeRepository`，17 个单测通过、ruff 通过；**Task 142（项目创建可携带大纲，`--outline-file` JSON 导入 + 原子写入）已完成**（`tasks/142-project-outline-import-DONE.md`，12 个单测通过）；**Task 143（GoalPlanner 自顶向下派生）已完成**（`tasks/143-goal-planner-topdown-derivation-DONE.md`，9 个单测通过）：新增骨架上下文加载器、工艺卡 1.1.0 注入弧目标/线索/伏笔、`ChapterGoal.derived_from_arc` 迁移+持久化、`goal_planner_node` 接线，无骨架逐字节回退；**Task 144（线索经济约束 MVP）已完成**（`tasks/144-thread-economy-mvp-DONE.md`，9 个单测通过）：CreativeDirector 工艺卡 1.0.6 注入线索经济约束、settlement 后 `update_plot_threads_after_settlement` 依证据推进 PlotThread 状态（`opened/advanced/resolved`，T1 可追溯）。**V6 阶段 0（Task 141-144）叙事骨架 MVP 工程实现全部完成**；阶段 0 出口 (b)(c) 的 Ch1-Ch20 真跑数据验证需带大纲长跑，转入阶段 D/后续长跑任务。**V6 阶段 A（Task 145-148 度量 + 148z 阈值标定）已全部完成**：新增 `evals/db_metrics.py` DB 支撑度量框架 + `songyan metrics` 命令（orphan 绝对量/T7 速率/质量债/文学趋势/弧级伏笔兑现率+长程台账六段，derive-on-read 可复算历史库）、`run_quality_debt` 表、literary 范围回读趋势查询；145-148 共 31 个单测通过；148z 用 138n/138k 复算冻结 T3/T6(a)(b)/T8、延后 T4/T5，报告 `docs/reports/v6-stageA-threshold-calibration.md`。新度量复现了被 health 8.5 掩盖的退化（orphan 斜率 +6.28/章、峰值 912、critical orphan 81、494 未兑现伏笔中 464 逾期归档）。**阶段 0/A 交付复审（2026-07-02）修复 3 处中危缺陷**：#1 `settlement_extractor_node` PlotThread 后处理异常捕获扩到 `SongyanError`/`sqlite3.Error`（守住非阻塞契约）、#2 `_upsert_quality_debt` 由逐章改为每 10 章 + 收尾兜底刷新（消除 O(n²) 长跑重算、补 `ValueError`/`JSONDecodeError` 捕获）、#3 `update_plot_threads_after_settlement` 改为单事务批量推进（一章多线索原子化）；全量 `pytest` 2099 passed 无回归，标注见 `tasks/144-thread-economy-mvp-DONE.md`/`tasks/146-quality-debt-ledger-DONE.md`。下一步 V6 阶段 B（末端治理 Task 149-152，须在骨架+度量落地后开工——现已满足）。
