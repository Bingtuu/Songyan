...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V7 阶段 W/X/Y 已通过，T9/T10/T12 已冻结；但 Task 170b 判定 Ch200 长跑 blocker，转入文学提质专项（170c–170g）**：Task 170b 中段窗口（Ch28–Ch40）真实实读证明"治理指标全达标 ≠ prose 好看"——T9=0、continuity health 9.1–9.7、QG 全 pass，而 voice 均值 1.8（塌陷）、节奏偏慢、Ch31 有真实文本缺陷，机器文学诊断系统性高估、T9 近似重复漏报。**Ch200（Task 171）暂缓**。提质专项遵循"量具优先"：**170c（T9 近似重复补强）✅ + 170d（LiteraryAuditor 校准）✅ 已完成**，下一步 170e（voice）/170f（pacing）提质 → 170g 复评出口 → 达 pass/observation 才放行 Ch200。专项总览 `tasks/170-literary-quality-remediation-README.md`。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2428 passed, 2 skipped, 1 xfailed, 2 warnings`（170c+170d 完成后全量；新增 test_170c_near_duplicate 7 + test_170d_auditor_calibration 7）；`ruff check src/ tests/ scripts/` 通过 |
...
| 当前 lint | `ruff check src/ tests/ scripts/` 已通过（170c+170d 完成后） |
| Python | 3.11.9 |
| 后续阶段 | 阶段 W/X/Y 已通过、T9/T10/T12 已冻结；但 **Task 170b 判定 Ch200 blocker**，阶段 Z 暂缓。当前在**文学提质专项（170c–170g）**：量具阶段 170c/170d 已完成，下一步 170e（voice 声纹提质）/170f（pacing/exposition），再 170g 复评出口；170g 达 pass/observation 才放行 Task 171 Ch200 长跑。 |
...
| 下一步规划 | **文学提质专项（170c–170g），非 Ch200**。Task 170b 判定 Ch200 blocker，Task 171 暂缓。量具阶段 170c（T9 近似重复补强）/170d（LiteraryAuditor 校准）已完成；下一步 170e（voice 声纹）/170f（pacing/exposition）提质 → 170g 用校准后量具复评中段窗口；170g 达 pass/observation 才放行 Task 171 Ch200。全程不放宽 T9/T10/T5/T6/T12 冻结口径。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **文学提质专项（阶段 Z 前置门，当前重心）**：Task 170b 中段窗口实读判定 Ch200 blocker——"治理指标全达标 ≠ prose 好看"实证成立（voice 均值 1.8 塌陷、节奏偏慢、Ch31 真实缺陷）。遵循"量具优先"：**170c（T9 近似重复补强）✅、170d（LiteraryAuditor 校准，回测 character_autonomy 均值 7.69→2.46、13/13 章向人工 voice 收敛）✅ 已完成**；下一步 170e（voice 声纹提质）/170f（pacing+exposition），再 170g 复评出口。170g 达 pass/observation 才放行 Task 171 Ch200。专项总览 `tasks/170-literary-quality-remediation-README.md`。
