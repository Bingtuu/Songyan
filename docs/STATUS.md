...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2270 passed, 2 skipped, 1 xfailed, 2 warnings`（Task 159 准备工作 +27 冒烟测试）；`ruff check` 通过 |
...
| 当前 lint | `ruff check src/ tests/ scripts/` 已通过（含 `scripts/run_159_ch1_ch150.py`、`scripts/supervise_159.py`，Task 159 完成后） |
| Python | 3.11.9 |
| 后续阶段 | **V6 全阶段完成（Task 141-159）**。阶段 D 收官：157（Ch1-Ch50 + harness）、158（Ch1-Ch100 + T5 首测）、158r（kill→resume 命令级证据）、159（Ch1-Ch150 复现 + V6 阶段验收）全部完成。**V6 阶段验收：实质达标、条件通过**（`run-bba292da` 150/150 accept、health≥8.2、T5 已冻结）。遗留为 harness 度量口径校准（非治理退化），列后续工程 Task：① `check_t5` 改中位数×2.0 + 稳健采样并落地冻结值；② `check_t6b` 对已 resolved 设定豁免历史 orphan 快照、`check_t6c` 小基数归因口径校准。下一步进入 **V7**（见 `docs/v7-vision.md`）。事实入口 `tasks/V6-README.md` |
...
1. **V6 阶段 D 收官（Task 159 已完成）**：Ch1-Ch150 治理管线复现无人值守长跑（run `run-bba292da`，隔离 DB `.tmp/task159_ch1_ch150.db`，enforce + isolate，真实 DeepSeek API，由自主督跑脚本 `scripts/supervise_159.py` 保守策略完成）。**结果：150/150 accept、`failed=[]`、health 全程 ≥8.2（无 <7.0）**；orphan 斜率 0.0897（≪138n 基线 6.28）、无 T3/T4 红线；N/D/R 三项 harness 直接 pass；中途多次 AutoHalt（Ch54 P1 orphan、Ch3/Ch13 resume 重算预算门禁）均由督跑器自动 resume 消解，额外实证 enforce 门禁下自愈续跑至 150/150。**T5 已冻结**：150 章 18 样本，DB 尺寸峰值 112.32MB（≤300MB）；扫描耗时口径由"前10均值×1.5"（缺陷、误判 9 处）改为"全样本中位数×2.0"（仅 Ch115 单点抖动），实质通过。**两处 harness fail 判为口径待校准项、非治理退化**：T6b（Ch54/57 的 critical orphan `第一代共鸣者遗骸` 最终已 `resolved`@Ch59，属瞬时窗口现象）、T6c（源头收敛过度小基数失真）。**V6 阶段验收结论：实质达标、条件通过**。报告 `docs/reports/task-159-v6-final-acceptance-report.md`，DONE `tasks/159-ch1-ch150-governance-pipeline-replication-DONE.md`。
2. **阶段 A 出口标定报告**：**已完成**（Task 148z）——T3/T6(a)(b)/T8 由 138n/138k 冻结；T4 在 157 首测通过；**T5 已在 Task 159 用 150 章 18 样本复核并冻结**（尺寸≤300MB + 扫描耗时中位数×2.0）。§1.4 全部阈值标定完成。
3. **后续工程 Task（V6 收尾后 / V7）**：① `check_t5` 默认参数改中位数×2.0 + 单点稳健采样并落地冻结值，同步更新 `tests/test_158_t5_freeze.py`；② `check_t6b` 对已 resolved 设定豁免历史 orphan 快照、`check_t6c` 小基数归因口径校准。均为 harness（度量）改进，不涉及生成/门禁治理逻辑。
