...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V7 阶段 W/X/Y 已通过，T9/T10/T12 已冻结；但 Task 170b 判定 Ch200 长跑 blocker，转入文学提质专项（170c–170g）**：Task 170b 中段窗口（Ch28–Ch40）真实实读证明"治理指标全达标 ≠ prose 好看"——T9=0、continuity health 9.1–9.7、QG 全 pass，而 voice 均值 1.8（塌陷）、节奏偏慢、Ch31 有真实文本缺陷，机器文学诊断系统性高估、T9 近似重复漏报。**Ch200（Task 171）暂缓**。提质专项遵循"量具优先"：**170c（T9 近似重复补强）✅ + 170d（LiteraryAuditor 校准）✅ + 170e（voice 声纹提质）✅ 已完成**——170e 定位 voice 塌陷根因为 **seeding gap**（characters 表为空致声纹机制全程死代码，非生成侧缺陷），用幂等 `ensure_protagonist_character` 修复并小样本验证机制激活（snapshot 声纹卡 0→1/3/3）。**170f（pacing/exposition）✅ 已完成（部分达标）**——Stage 0 校准证明代码指标无法可靠区分 pacing，转为纯生成侧约束（Writer 1.1.0 新增 `scene_interaction` 段落，约束单人解谜/无他者互动）；Stage 2 小样本重生成 Ch29–Ch32 验证：**pacing 3.25 ✅ 达标（目标 ≥3.0），exposition 2.0 ❌ 未达标（目标 ≥2.5）**；实际通读发现模型用"意识触须/信息流读取 + 幻象播放 + FAQ 式 NPC 问答"绕过互动约束，说明文载体未受控。DONE 文档 `tasks/170f-pacing-exposition-DONE.md`、评估报告 `docs/reports/task-170f-stage2-reeval-report.md`。下一步 **170g** 接续：追加 exposition 载体约束（禁信息流硬灌、限 FAQ 式对话、动作承载信息、增加留白），复评 Ch28–Ch40 中段窗口；170g 达 pass/observation 才放行 Ch200。专项总览 `tasks/170-literary-quality-remediation-README.md`。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2435 passed, 2 skipped, 1 xfailed`（170f Stage 1 完成后全量；新增 170f 约束无测试回归） |
...
| Python | 3.11.9 |
| 后续阶段 | 阶段 W/X/Y 已通过、T9/T10/T12 已冻结；但 **Task 170b 判定 Ch200 blocker**，阶段 Z 暂缓。当前在**文学提质专项（170c–170g）**：量具阶段 170c/170d 已完成，提质阶段 **170e（voice）✅ 已完成**（根因 seeding gap，已修复并验证机制激活），**170f（pacing/exposition）✅ 已完成（部分达标）**——Stage 2 小样本验证 pacing 3.25 ✅、exposition 2.0 ❌，实际通读发现模型用"意识触须/信息流读取 + 幻象播放 + FAQ 式 NPC 问答"绕过互动约束，说明文载体未受控；DONE 文档 `tasks/170f-pacing-exposition-DONE.md`、报告 `docs/reports/task-170f-stage2-reeval-report.md`。下一步 **170g** 追加 exposition 载体约束（禁信息流硬灌、限 FAQ 式对话、动作承载信息、增加留白），复评 Ch28–Ch40 中段窗口；170g 达 pass/observation 才放行 Task 171 Ch200 长跑。 |
...
| 下一步规划 | **文学提质专项（170c–170g），非 Ch200**。Task 170b 判定 Ch200 blocker，Task 171 暂缓。量具阶段 170c/170d 已完成；提质阶段 **170e（voice）✅ 已完成**（根因 seeding gap：characters 表为空致声纹机制死代码，已用 `ensure_protagonist_character` 修复并验证激活）；**170f（pacing/exposition）✅ 已完成（部分达标）**——Stage 0 校准负结果→转纯生成侧，Writer 1.1.0 新增 `scene_interaction` 段落；Stage 2 小样本验证 pacing 3.25 ✅、exposition 2.0 ❌，实际通读确认模型用"意识触须/信息流读取 + 幻象播放 + FAQ 式 NPC 问答"绕过互动约束，说明文载体未受控。下一步 **170g** 追加 exposition 载体约束（禁信息流硬灌、限 FAQ 式对话、动作承载信息、增加留白），用校准后量具复评 Ch28–Ch40 中段窗口；170g 达 pass/observation 才放行 Task 171 Ch200。全程不放宽 T9/T10/T5/T6/T12 冻结口径。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **文学提质专项（阶段 Z 前置门，当前重心）**：Task 170b 中段窗口实读判定 Ch200 blocker——"治理指标全达标 ≠ prose 好看"实证成立（voice 均值 1.8 塌陷、节奏偏慢、Ch31 真实缺陷）。遵循"量具优先"：**170c（T9 近似重复补强）✅、170d（LiteraryAuditor 校准，回测 character_autonomy 均值 7.69→2.46、13/13 章向人工 voice 收敛）✅、170e（voice 声纹提质）✅ 已完成**——170e 定位 voice 塌陷根因为 seeding gap（characters 表为空致声纹机制全程死代码），用幂等 `ensure_protagonist_character` 在 CLI 建项目+流水线启动补建 protagonist，小样本真实生成验证 snapshot 声纹卡 0→1/3/3、主角卡自动生成；**170f（pacing/exposition）✅ 已完成（部分达标）**——Stage 0 校准证明 4 个候选代码指标全部无法可靠区分 pacing 好/慢章（诚实结论），转为纯生成侧约束（Writer 1.1.0 新增 `scene_interaction` 段落，约束每章至少 1 个多人互动场景、连续单人叙述上限 500 字、禁连续 2+ 纯单人场景、信息通过互动释放）；Stage 2 小样本重生成 Ch29–Ch32 验证 **pacing 3.25 ✅ 达标（目标 ≥3.0）、exposition 2.0 ❌ 未达标（目标 ≥2.5）**；实际通读发现模型用"意识触须/信息流读取 + 幻象播放 + FAQ 式 NPC 问答"绕过互动约束，说明文载体未受控，DONE 文档 `tasks/170f-pacing-exposition-DONE.md`、报告 `docs/reports/task-170f-stage2-reeval-report.md`。下一步 **170g** 追加 exposition 载体约束（禁信息流硬灌、限 FAQ 式对话、动作承载信息、增加留白），复评 Ch28–Ch40 中段窗口；170g 达 pass/observation 才放行 Task 171 Ch200。专项总览 `tasks/170-literary-quality-remediation-README.md`。
2. **V6 阶段验收（Task 159 已完成，前置基线）**：Ch1-Ch150 治理管线复现无人值守长跑（run `run-bba292da`，隔离 DB `.tmp/task159_ch1_ch150.db`，enforce + isolate，真实 DeepSeek API，由自主督跑脚本 `scripts/supervise_159.py` 保守策略完成）。**结果：150/150 accept、`failed=[]`、health 全程 ≥8.2**；orphan 斜率 0.0897（≪138n 基线 6.28）、无 T3/T4 红线；N/D/R 三项 harness 直接 pass；**T5 已冻结**（150 章 18 样本，DB 峰值 112.32MB，扫描耗时口径改"全样本中位数×2.0"）。**V6 阶段验收结论：实质达标、条件通过**。报告 `docs/reports/task-159-v6-final-acceptance-report.md`。
3. **后续工程 Task（V7 期间处理）**：① `check_t5` 默认参数改中位数×2.0 + 单点稳健采样并落地冻结值，同步更新 `tests/test_158_t5_freeze.py`；② `check_t6b` 对已 resolved 设定豁免历史 orphan 快照、`check_t6c` 小基数归因口径校准。均为 harness（度量）改进，不涉及生成/门禁治理逻辑。
4. **持续回归**：后续改动继续执行 `pytest tests/ -q` + `ruff check src/ tests/`。\r
\r
