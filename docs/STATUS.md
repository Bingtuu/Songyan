...
| 下一步规划 | 重跑 Ch1-Ch150 single-run rehearsal，验证 Ch5 是否解除阻断 |

测试说明：`2 xfailed` 为已知非阻断项，其中包含 Windows SQLite 并发写入限制，以及冷启动下 embedding model 加载导致的性能 xfail。

## 当前优先级

1. 重跑 Ch1-Ch150 single-run rehearsal，验证 Task 121c 是否解除 Ch5 settlement skip 阻断。
...
| 项 | 级别 | 处理 |
|----|------|------|
| 一次性 Ch1-Ch150 单命令证据 | P1 | **Task 121b 已执行但未通过；Task 121c 已修复直接阻断**。需要重跑 rehearsal 验证 |
