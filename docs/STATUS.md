...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V7 阶段 W/X/Y 已通过，Task 166/167/168/169/170 已完成，T12 已冻结，下一步规划 171 Ch200 长跑**：Task 170 已用良性波动/真实退化/单点毛刺/单域四类小窗口验证 168/169 自适应门禁——良性 FP rate=0、真实退化 halt_candidate/halt=100%、可读性/文学性抽检无 blocker，未改 T9/T10/T5/T6 口径、未启动 Ch200；报告 `docs/reports/task-170-adaptive-gate-validation-report.md`，DONE `tasks/170-...-DONE.md`。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2414 passed, 2 skipped, 1 xfailed, 2 warnings`（Task 170 完成后全量测试，新增 17 个 Task 170 测试）；`ruff check src/ tests/ scripts/run_170_adaptive_gate_validation.py` 通过 |
...
| 当前 lint | `ruff check src/ tests/ scripts/run_170_adaptive_gate_validation.py` 已通过（Task 170 完成后） |
| Python | 3.11.9 |
| 后续阶段 | 阶段 W（质量修复）已通过；阶段 X（叙事自驱 166-167）已完成；阶段 Y（enforce 可生产化 168-170）已完成——Task 170 完成 enforce 小窗口验证 + T12 冻结；后续进入：**阶段 Z（Ch200→250→300 渐进爬坡）**。下一步规划 Task 171 Ch200 长跑（171 首窗需做真实正文抽读）。 |
...
| 下一步规划 | **Task 171 规划：Ch200 长跑（V7 第一里程碑）**。Task 170 已完成 enforce 小窗口验证 + T12 冻结（良性 FP rate=0、真实退化拦截率 100%）；171 首窗需做真实正文抽读，仍不放宽 T9/T10/T5/T6/T12 冻结口径。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **Task 171 规划（阶段 Z 启动）**：Task 170 已完成并冻结 T12。下一步规划 Ch200 长跑，首窗做真实正文抽读，继续遵守“不放宽 T9/T10/T5/T6/T12 冻结口径”的边界。
