...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V7 阶段 W 已通过，Task 166 已完成，下一步规划 Task 167**：Task 166a 已提供离线弧后评估与 draft `ReplanProposal` 生成能力；Task 166b 已实现人工确认后的事务化应用。re-plan 默认不改正文、不接入主 workflow，且 Task 165/165p 的 T9/T10 冻结口径未改动。Task 166 总结见 `tasks/166-plan-generate-replan-loop-DONE.md`。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2352 passed, 2 skipped, 1 xfailed, 2 warnings`（Task 166b 完成后全量测试）；`ruff check src/ tests/` 通过 |
...
| 当前 lint | `ruff check src/ tests/` 已通过（Task 166b 完成后）；Task 159 后 `ruff check src/ tests/ scripts/` 亦曾通过 |
| Python | 3.11.9 |
| 后续阶段 | 阶段 W（质量修复）已通过；后续依次推进：**阶段 X（叙事自驱 166-167，plan→re-plan 闭环 + 伏笔调度）→ 阶段 Y（enforce 可生产化 168-170，门禁自适应）→ 阶段 Z（Ch200→250→300 渐进爬坡 171-173，每级带 `NNNp` 撞墙修复占位）**。下一步规划 Task 167，不直接进入 Ch200。 |
...
| 下一步规划 | **Task 167 规划：长程伏笔主动兑现调度**。Task 166 已建立可审计、可回滚的 re-plan 提案与应用能力；167 应基于该能力规划主动伏笔调度，不直接做自动正文改写。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **Task 167 规划（阶段 X 继续）**：Task 166 已完成离线评估、draft proposal、人工确认后的事务化应用。下一步规划长程伏笔主动兑现调度，继续遵守“不新增不可控自动改写闭环、可审计可回滚”的边界。
