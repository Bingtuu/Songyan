...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V6 进行中**：阶段 0+A+B 已完成；阶段 C 工程实现已完成；**阶段 D：Task 157 已完成（157a harness + 32 单测；157b Ch1-Ch50 实跑已执行，run `run-e4528e8c`，48/50 accept，无 AutoHalt/门禁触发），158 Ch100 长跑待执行**。Task 141-159，事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md`，论证基础 `docs/300-chapter-gap-analysis.md`。前置 V5.2 已完成验收（下行）。 |
...
1. **V6 阶段 D 进行中**：**Task 157 已完成**——157a V6 验收判据 harness（`src/songyan/evals/v6_acceptance.py` + `tests/test_157_v6_acceptance.py` 32 个单测）+ 157b Ch1-Ch50 无人值守实跑（run `run-e4528e8c`，隔离 DB `.tmp/task157_ch1_ch50.db`，enforce + isolate，真实 DeepSeek API，~3.7h）。**结果：48/50 accept，无 AutoHalt/门禁/ContextEmergency**；harness 判定 T1/T6a/T3/T8/T4/T5(采样)/health≥7.0 全过，T6b 实测 P1=0（16 审计点 orphan_critical 全 0）；两项未过均非治理退化——**T2=48/50**（Ch14 settlement 数值幻觉被硬校验正确拦截、Ch27 正文偏短 QG 未过）、**T6c** 因新 critical 速率被骨架压到 ≈0（0.02/章）致归因比值在退化基线下算术失真（实为源头收敛过度达标）。报告 `docs/reports/task-157-ch1-ch50-integration-validation-report.md`。**下一步：Task 158 Ch1-Ch100 长跑 + kill→resume + T5 冻结（复用 157a harness）**；后续可选 `--resume` 定点复跑 Ch14/Ch27 确认 settlement 抖动是否偶发。
