...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V7 启动（阶段 W 详细文档已落地、待开工）**：V7 从 Ch150 篇章级质量修复爬坡至 Ch300，分 W（质量修复 160-165）/X（叙事自驱 166-167）/Y（enforce 可生产化 168-170）/Z（Ch200→300 爬坡 171-173 + `NNNp` 撞墙修复占位）四阶段共 17 Task。阶段 W 六个详细 Task 文档（160 元标记根治 / 161 段落去重 / 162 时间线一致 / 163 概念预算 / 164 洁净度度量 / 165 Ch150复跑+T9/T10冻结）已写就，X/Y/Z 为骨架占位待 W 出口数据后写。规划 `docs/v7-plan.md`、构想 `docs/v7-vision.md`、事实入口 `tasks/V7-README.md`。**驱动依据**：对 V6 `run-bba292da` 全 150 章正文人工通读暴露 52 章元标记泄漏 / 19 章整段重复+时间线矛盾 / conceptual_grounding 7.12→6.02 概念通胀等 V6 句子级度量掩盖的篇章级质量债。 |
...
| 下一步规划 | **V7 阶段 W 开工**：从 Task 160（元标记泄漏根治）起顺序推进 160-165，出口以 Task 165（Ch1-Ch150 修复后复跑 + T9/T10 冻结）为闸门。阶段 W 全部达标后再进 X（叙事自驱）/Y（enforce 可生产化）/Z（Ch200→300 爬坡）。规划 `docs/v7-plan.md`，事实入口 `tasks/V7-README.md`。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **V7 阶段 W 开工（篇章级质量修复，Task 160-165）**：六个详细 Task 文档已落地——160 元标记泄漏根治（正则补全 + Writer 强制清洗 + RuleAuditor severity 升为阻塞）、161 段落级去重（`_reassemble_content` 拼接侧去重 + 重复长段落检测）、162 跨章时间线一致性检测（先诊断不阻塞）、163 概念预算约束（规划侧限流 + 落地优先，不自动改写）、164 文本洁净度度量入库 + T9 harness、165 阶段 W 出口（Ch1-Ch150 复跑 + T9/T10 标定冻结）。**驱动依据**：对 V6 `run-bba292da` 全 150 章正文人工通读暴露 52 章元标记泄漏 / 19 章整段重复+时间线矛盾（Ch75 最重）/ conceptual_grounding 7.12→6.02 概念通胀——均为 V6 句子级度量掩盖的篇章级质量债。规划 `docs/v7-plan.md`，事实入口 `tasks/V7-README.md`。下一步：从 Task 160 开工。
