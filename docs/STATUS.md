...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V6 进行中**：阶段 0（叙事骨架 MVP，Task 141-144）+ 阶段 A（长篇质量度量，Task 145-148 + 148z）**已完成**；**阶段 B 工程实现已收口（Task 149-152）**；**阶段 C 已完成：Task 153 run 级断点续跑、Task 154 LLM 限流感知与全局预算、Task 155 失败隔离策略、Task 156 运行中 DB 维护均已完成**。Task 141-159，事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md`，论证基础 `docs/300-chapter-gap-analysis.md`。前置 V5.2 已完成验收（下行）。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2187 passed, 2 skipped, 1 xfailed, 1 warning`（阶段 C 完成后）；`ruff check src/ tests/` 通过 |
...
| Python | 3.11.9 |
| 后续阶段 | **V6 阶段 C（工程加固，Task 153-156）已完成**：Task 153 run 级断点续跑（13 个新单测）、Task 154 LLM 限流感知与全局预算（12 个新单测）、Task 155 失败隔离策略（`on_failure` 默认改 `isolate`，7 个新单测）、Task 156 运行中 DB 维护（11 个新单测）均已合入。**下一步进入阶段 D（长窗口验证，Task 157-159）**。事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md` |
...
| 下一步规划 | **V6 阶段 C（工程加固 Task 153-156）已完成**，全量 pytest `2187 passed, 2 skipped, 1 xfailed`；下一步进入 **阶段 D（长窗口验证 157-159）**，取得 Ch1-Ch150 连续运行证据。事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md` 与 `docs/v7-vision.md`。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **V6 阶段 C（工程加固）已完成**：**Task 153 run 级断点续跑**（13 个新单测）、**Task 154 LLM 限流感知与全局预算**（12 个新单测）、**Task 155 失败隔离策略**（`on_failure` 默认改 `isolate`，7 个新单测）、**Task 156 运行中 DB 维护**（周期 wal_checkpoint + optimize + DB/WAL 尺寸与扫描耗时遥测入库，11 个新单测）均已完成。下一步进入阶段 D（长窗口验证 157-159），取得 Ch1-Ch150 连续运行证据。事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md`。
