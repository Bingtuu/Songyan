...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V7 阶段 W 已通过，Task 166/167 已完成，下一步规划 Task 168**：167 已将长程伏笔从被动审计推进到主动调度，系统能生成 draft `ForeshadowingSchedulePlan`，并将 active 调度项注入 GoalPlanner / CreativeDirector 的规划侧输入；章节 accept 后会推进 `injected/satisfied/missed` 生命周期。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2368 passed, 2 skipped, 1 xfailed, 2 warnings`（Task 167b 完成后全量测试）；`ruff check src/ tests/ scripts/run_167a_foreshadowing_schedule.py` 通过 |
...
| 当前 lint | `ruff check src/ tests/ scripts/run_167a_foreshadowing_schedule.py` 已通过（Task 167b 完成后）；Task 159 后 `ruff check src/ tests/ scripts/` 亦曾通过 |
| Python | 3.11.9 |
| 后续阶段 | 阶段 W（质量修复）已通过；阶段 X（叙事自驱 166-167）已完成；后续依次推进：**阶段 Y（enforce 可生产化 168-170，门禁自适应）→ 阶段 Z（Ch200→250→300 渐进爬坡 171-173，每级带 `NNNp` 撞墙修复占位）**。下一步规划 Task 168，不直接进入 Ch200。 |
...
| 下一步规划 | **Task 168 规划：自适应门禁数据面**。Task 167 已提供 schedule lifecycle 信号；168 应将调度命中率、missed rate、overdue rate、质量债趋势等沉淀为 gate 可读数据面，为 169/170 的自适应 halt 和 T12 标定提供输入。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **Task 168 规划（阶段 Y 启动准备）**：Task 167 已完成主动伏笔调度和生命周期推进。下一步把 schedule lifecycle、质量债趋势、orphan/health 滑窗等信号整理为自适应门禁数据面，继续遵守“不直接放宽冻结阈值、不启动 Ch200”的边界。
