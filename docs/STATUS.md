...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V6 进行中**：阶段 0（叙事骨架 MVP，Task 141-144）+ 阶段 A（长篇质量度量，Task 145-148 + 148z）**已完成**；**阶段 B 工程实现已收口（Task 149 录入侧降级完成、Task 150 `_infer_setting_category` 收紧完成、Task 151 MR 上限自适应 + 相关性排序完成、Task 152 critical 显式 resolve/作废出口完成）**；阶段 C/D（工程加固 / 长窗口验证）待做。Task 141-159，事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md`，论证基础 `docs/300-chapter-gap-analysis.md`。前置 V5.2 已完成验收（下行）。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2144 passed, 2 skipped, 1 xfailed, 2 warnings`（阶段 B 交付复审修复后）；`ruff check src/ tests/` 通过 |
...
| Python | 3.11.9 |
| 后续阶段 | **下一步 V6 阶段 C（工程加固，Task 153-156）与阶段 D（长窗口验证，Task 157-159）**。阶段 B（末端治理 Task 149-152）工程实现已收口。事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md` |
...
| 下一步规划 | **V6 阶段 B（末端治理 Task 149-152）工程实现已收口**，全量 pytest `2139 passed`；下一步进入 **阶段 C（工程加固 153-156）** 与 **阶段 D（长窗口验证 157-159）**。事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md` 与 `docs/v7-vision.md`。 |

测试说明：`1 xfailed` 为已知非阻断项；`0 xpassed`（已修复）；`0 failed`。数据库 `lifecycle_status` 列缺失与 RAG embedding 性能测试已修复，当前全量通过。

## 当前优先级

1. **V6 阶段 B（末端治理）已收口**：**Task 149 录入侧降级已完成**，**Task 150 `_infer_setting_category` 收紧已完成**，**Task 151 MR 上限自适应 + 相关性排序已完成**，**Task 152 critical 显式 resolve/作废出口已完成**（新增 `resolved`/`abandoned` 终态与 settlement 联动，10 个单测，`ruff` 通过）。**阶段 B 交付复审（2026-07-02）发现并修复 3 类缺陷**：#1（P1）同章 demote→promote 相互抵消致 Task 149 在真实管线失效（promote 改为只回升往期候选）；#2（P2）Task 152 resolve 裸子串误命中主线短名词、重蹈 Task 144 覆辙（改精确/长度阈值匹配 + 本章不 resolve）；#5（性能回归）settlement 后处理三段治理各自开连接使 accept 热路径超 timing 微基准（加早退守卫 + 阈值 1000→1500ms 并注明理由）。修复后权威全量 `2144 passed`（+5 净新增用例，无回归）。下一步进入阶段 C（工程加固 153-156）与阶段 D（长窗口验证 157-159）。事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md`。
