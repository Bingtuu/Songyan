...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V7 阶段 W 已开工（Task 160-164 完成，Task 165 待开工）**：V7 从 Ch150 篇章级质量修复爬坡至 Ch300，分 W（质量修复 160-165）/X（叙事自驱 166-167）/Y（enforce 可生产化 168-170）/Z（Ch200→300 爬坡 171-173 + `NNNp` 撞墙修复占位）四阶段共 17 Task。Task 160 已完成元标记泄漏根治；Task 161 已完成段落级去重；Task 162 已完成跨章时间线一致性诊断；Task 163 已完成概念预算约束；Task 164 已完成文本洁净度度量入库 + `songyan metrics` 展示 + T9 harness：新增 `text_cleanliness_metrics`、accepted 正文洁净度复算入库、T9 三态判据（时间线默认 report-only，待 165 冻结）。全量 `2317 passed, 2 skipped, 1 xfailed`，`ruff check src/ tests/` 通过。下一步 Task 165 阶段 W 出口复跑；X/Y/Z 为骨架占位待 W 出口数据后写。规划 `docs/v7-plan.md`、构想 `docs/v7-vision.md`、事实入口 `tasks/V7-README.md`。**驱动依据**：对 V6 `run-bba292da` 全 150 章正文人工通读暴露 52 章元标记泄漏 / 19 章整段重复+时间线矛盾 / conceptual_grounding 7.12→6.02 概念通胀等 V6 句子级度量掩盖的篇章级质量债。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2317 passed, 2 skipped, 1 xfailed, 2 warnings`（Task 164 完成后全量测试）；`ruff check src/ tests/` 通过 |
...
| 当前 lint | `ruff check src/ tests/` 已通过（Task 164 完成后）；Task 159 后 `ruff check src/ tests/ scripts/` 亦曾通过 |
| Python | 3.11.9 |
...
| 下一步规划 | **V7 阶段 W 出口 Task 165**：Task 160-164 已完成；下一步执行 Task 165（Ch1-Ch150 修复后复跑 + T9/T10 标定冻结）。阶段 W 出口以真实 Ch150 证据核验元标记 52→0、重复段落 19→0、时间线矛盾收敛、conceptual_grounding 止跌，并冻结 T9/T10 正式口径；通过后再进 X/Y/Z。规划 `docs/v7-plan.md`，事实入口 `tasks/V7-README.md`。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **V7 阶段 W 出口准备（Task 165 待开工）**：Task 160 元标记泄漏根治已完成（DONE `tasks/160-meta-tag-leak-eradication-DONE.md`）；Task 161 段落级去重已完成（DONE `tasks/161-paragraph-dedup-DONE.md`）；Task 162 跨章时间线一致性诊断已完成（DONE `tasks/162-cross-chapter-timeline-consistency-DONE.md`）；Task 163 概念预算约束已完成（DONE `tasks/163-concept-budget-constraint-DONE.md`）；Task 164 文本洁净度度量 + T9 harness 已完成（DONE `tasks/164-text-cleanliness-metrics-DONE.md`）。下一步 Task 165：Ch1-Ch150 修复后复跑 + T9/T10 标定冻结。**驱动依据**：对 V6 `run-bba292da` 全 150 章正文人工通读暴露 52 章元标记泄漏 / 19 章整段重复+时间线矛盾（Ch75 最重）/ conceptual_grounding 7.12→6.02 概念通胀——均为 V6 句子级度量掩盖的篇章级质量债。规划 `docs/v7-plan.md`，事实入口 `tasks/V7-README.md`。
