...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V7 阶段 W Task 165 已完成真实 Ch1-Ch150 复跑，但阶段 W 出口仍条件不通过**：`run-11fc7c96` 在外部中断后 resume，最终 150/150 accepted、`failed=[]`、无 AutoHalt；P 洁净 pass（元标记 52→0、重复段落 19→0，时间线 3 章 report-only）、L 文学 pass（conceptual_grounding 首段 W=5 6.80，末段 W=5 6.06，阈值 5.78）。阻断项为“不回退”：T5 扫描耗时旧口径 fail、T6c 归因口径 fail。报告 `docs/reports/task-165-stage-w-exit-report.md`；T9/T10 标定候选见 `docs/reports/task-165-v7-threshold-calibration.md`。下一步先处理 V6 遗留 harness 口径（T5/T6b/T6c），不得进入 X/Y/Z。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2336 passed, 2 skipped, 1 xfailed, 2 warnings`（Task 165 resume/source-of-truth + 重复段落门禁修复后全量测试）；`ruff check src/ tests/` 通过 |
...
| 下一步规划 | **V7 阶段 W 出口阻断项处理**：Task 165 真实 Ch150 复跑已取得 150/150 accepted，P/L/T9/T10 达标；但阶段 W 总结论仍因 T5/T6c 不回退 harness fail 条件不通过。下一步优先校准 `check_t5` 扫描耗时口径与 `check_t6b/check_t6c` 小基数/已 resolved 口径，复算 Task 165 报告后再决定是否冻结 T9/T10 并进入 X/Y/Z。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **V7 阶段 W 出口阻断项处理（Task 165 后续）**：Task 165 真实 Ch1-Ch150 复跑 `run-11fc7c96` 已完成 150/150 accepted，P 洁净 / L 文学 / 修复对比通过；T9 候选口径为元标记=0、重复长段落=0、时间线 report-only，T10 ×0.85 通过。当前不得进入 X/Y/Z，需先处理 T5 扫描耗时旧口径 fail 与 T6c 归因口径 fail，再复算阶段 W 出口报告。
