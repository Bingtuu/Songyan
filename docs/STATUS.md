...
| 项 | 状态 |
|----|------|
| 当前阶段 | **V6 进行中**：阶段 0+A+B 已完成；阶段 C 工程实现已完成；**阶段 D：Task 157 已完成（157a harness + 32 单测；157b Ch1-Ch50 实跑已执行，run `run-e4528e8c`，48/50 accept，无 AutoHalt/门禁触发）；Task 158 已完成（run `run-10d7961b`，Ch1-Ch100 100/100 完成，98/100 accept，无 AutoHalt；T5 尺寸未破线、扫描耗时 Ch50/Ch70 破线，待 Task 159 复核后冻结；kill→resume 已由 Task 158r 补齐真实命令级证据，run `run-82bd2e07`）**。Task 141-159，事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md`，论证基础 `docs/300-chapter-gap-analysis.md`。前置 V5.2 已完成验收（下行）。 |
...
| 风险口径 | P0/P1 风险为 0 |
| 最近全量测试 | `2243 passed, 2 skipped, 1 xfailed, 2 warnings`（Task 158r kill→resume 演练后，+9 冒烟测试）；`ruff check` 通过 |
...
| 当前 lint | `ruff check src/ tests/ scripts/` 已通过（含 `scripts/run_158r_kill_resume_drill.py`，Task 158r 完成后） |
| Python | 3.11.9 |
| 后续阶段 | **V6 阶段 C（工程加固，Task 153-156）已完成**：Task 153 run 级断点续跑（13 个新单测）、Task 154 LLM 限流感知与全局预算（12 个新单测）、Task 155 失败隔离策略（`on_failure` 默认改 `isolate`，7 个新单测）、Task 156 运行中 DB 维护（11 个新单测）均已合入。**阶段 D（长窗口验证，Task 157-159）：157 已完成（157a harness + 157b Ch1-Ch50 实跑 run `run-e4528e8c` 48/50 accept，无熔断）；158 已完成（run `run-10d7961b` Ch1-Ch100 98/100 accept，T5 待 159 复核冻结，kill→resume 已由 Task 158r 补齐真实命令级证据 run `run-82bd2e07`）；159 Ch150 复现 + V6 阶段验收 待执行**。事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md` |
...
| 下一步规划 | **V6 阶段 C（工程加固 Task 153-156）已完成**；**阶段 D 已启动**：157a V6 验收判据 harness 已交付并配套 32 个 Layer 2 单测，全量 pytest `2234 passed, 2 skipped, 1 xfailed`，`ruff check` 通过；157b Ch1-Ch50 无人值守实跑已执行；**158 Ch1-Ch100 长跑已完成**（run `run-10d7961b`，100/100 完成，98/100 accept，报告 `docs/reports/task-158-ch1-ch100-long-run-validation-report.md`），T5 首次实测未冻结（Ch50/Ch70 扫描耗时破线，待 159 复核），kill→resume 已由 Task 158r 补齐真实命令级证据（run `run-82bd2e07`，报告 `docs/reports/task-158r-kill-resume-drill-report.md`）。后续 **159 Ch150 复现并产出 V6 验收报告**；事实入口 `tasks/V6-README.md`，规划 `docs/v6-plan.md` 与 `docs/v7-vision.md`。 |

测试说明：`1 xfailed` 为已知非阻断项；`2 skipped` 为 integration 中依赖外部数据的测试；`0 xpassed`（已修复）；`0 failed`。当前全量通过。

## 当前优先级

1. **V6 阶段 D 进行中**：**Task 158 已完成**——Ch1-Ch100 无人值守长跑（run `run-10d7961b`，隔离 DB `.tmp/task158_ch1_ch100.db`，enforce + isolate，真实 DeepSeek API，≈7.8h）。**结果：100/100 完成，98/100 accept，无 AutoHalt/门禁触发**；harness 判定 T1/T6a/T4/health≥7.0 通过；**T2=98/100**（Ch48、Ch62 settlement/summary 失败被 isolate）；**T3/T8** 文学趋势在 Ch10 起触线（literary/conceptual/fissure）；**T5** DB 尺寸 84.78MB 未破线，但扫描耗时在 Ch50/Ch70 达 1.93×/1.76× 基线而破线，按 148z 纪律需 Task 159 复核后再冻结；**T6c** 因新 critical 速率被骨架压到 0，归因比值判据在退化基线下算术失真（与 157 同因）。报告 `docs/reports/task-158-ch1-ch100-long-run-validation-report.md`，DONE 文档 `tasks/158-ch1-ch100-long-run-validation-DONE.md`。**§1.3-R kill→resume 已补齐真实命令级证据（Task 158r）**：全新隔离 DB `.tmp/task158r_kill_resume.db`，真实 API，`--kill-at-chapter 3` 在 Ch3 生成后 accept 前 in-flight kill（`run-82bd2e07`，accepted=[1,2]），同命令 `--resume` 复用同一 run_id、`resume_start=3`、prune 58 孤儿 checkpoint、Ch3 重算 + Ch4/Ch5 续完，最终 accepted=[1,2,3,4,5]、`status=completed`，5 项断言全过；证据 `docs/reports/task-158r-kill-resume-drill-report.md`。**下一步：Task 159 Ch150 复现 + V6 阶段验收**（可直接引用 158r 的 R 证据），并把 T5 阈值复核纳入 159 报告。
2. **阶段 A 出口标定报告**：**已完成**（Task 148z，`docs/reports/v6-stageA-threshold-calibration.md`）——用 138n/138k 历史 DB 复算冻结 v6-plan §1.4 的 T3/T6(a)(b)/T8；T4 在 Task 157 首次实测通过；T5 在 Task 158 首次实测尺寸未破线、扫描耗时破线，待 Task 159 复核调整后冻结。阶段 A 出口达成，末端治理（阶段 B）已解锁。
