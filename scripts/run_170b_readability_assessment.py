"""Task 170b: Ch28-Ch40 抽读评估（文学性/可读性初筛 + 机器对照）.

用法:
    # 在生成脚本跑完 Ch1-Ch40 后执行（复用同一隔离 DB）
    python scripts/run_170b_readability_assessment.py

    # 可指定窗口 / 项目
    $env:ASSESS_START="28"; $env:ASSESS_END="40"
    python scripts/run_170b_readability_assessment.py --project-id <pid>

    # 跳过 LLM 初评（只导出正文 + 机器分 + 洁净度，供纯人工实读）
    python scripts/run_170b_readability_assessment.py --no-llm

说明:
    - 助手初筛：抽 Ch28-Ch40 accepted 正文 + LiteraryAuditor 四维机器分 + T9 洁净度
      + run_log 关键字段；按 5 维 rubric 做 LLM 初评，标出可疑段落与机器/人工偏差候选。
    - 用户复核：读 docs/reports/task-170b-... 中标出的重点，给 5 维终评分。
    - 不改生成侧任何 Agent；不改 accept 行为；不放宽任何冻结口径。
    - 正文导出到 .tmp/task170b_prose_ch28_ch40.md（供人工逐章实读）。
"""
...
settings.database_url = os.getenv(
    "DATABASE_URL", "sqlite:///.tmp/task170b_ch1_ch40.db"
)

PROJECT_FILE = Path(".tmp/task170b_project.json")
REPORT_PATH = Path("docs/reports/task-170b-literary-readability-assessment-report.md")
PROSE_EXPORT_PATH = Path(".tmp/task170b_prose_ch28_ch40.md")
...
def _export_prose(chapters: dict[int, dict[str, Any]]) -> None:
    ...
    lines: list[str] = [f"# Task 170b 抽读正文 Ch{ASSESS_START}-Ch{ASSESS_END}", ""]
...
def _write_report(
    ...
    lines.append(
        ...
        "最终判定需用户读完第 5 节终评分后，在本报告或 170b DONE 文档中给出。"
    )
    lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Task 170b Ch28-Ch40 抽读评估")
