"""Task 170f Stage 2: pacing/exposition 提质后小样本重生成（Ch1-Ch32）.

用法:
    # 1. 初始化干净隔离 DB + 创建带大纲项目（复用 157b 的科幻大纲种子）
    python scripts/run_170f_stage2_generation.py --init

    # 2. 真实 LLM 跑 Ch1-Ch32（enforce 门禁，on_failure=isolate）
    python scripts/run_170f_stage2_generation.py

    # 中途 kill / AutoHalt 后续跑（复用同一 run_id，跳过已 accepted 章）
    python scripts/run_170f_stage2_generation.py --resume

说明:
    - 复用 157b 的脚手架与科幻大纲种子（轨道蜃景 / 林渊 / 方舟-共鸣-旧日搭档三主线）。
    - 本脚本用于验证 Writer 1.1.0 `scene_interaction` 段落对 Ch29-Ch32 pacing/exposition 的改善。
    - 生成后由 scripts/run_170f_stage2_reeval.py 做抽读评估，本脚本只负责生成。
    - 逐章 metrics 追加到 .tmp/task170f_stage2_ch1_ch32_metrics.jsonl。
"""
...
DB_PATH = Path(".tmp/task170f_stage2_ch1_ch32.db")
...
METRICS_PATH = Path(".tmp/task170f_stage2_ch1_ch32_metrics.jsonl")
...
async def main() -> None:
    ...
    print("下一步: python scripts/run_170f_stage2_reeval.py")
