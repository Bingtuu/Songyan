"""Task 170g: 中段窗口 Ch1-Ch40 提质后重生成（文学提质专项出口验证）.

用法:
    # 1. 初始化干净隔离 DB + 创建带大纲项目（复用 157b/170b 科幻大纲种子）
    python scripts/run_170g_generation.py --init
    ...
    python scripts/run_170g_generation.py --resume

说明:
    - 复用 157b/170b 的脚手架与科幻大纲种子（轨道蜃景 / 林渊 / 方舟-共鸣-旧日搭档三主线），
      保证与 170b 基线同种子、可比。
    - 本脚本加载 Task 170g 新增说明文载体约束（Writer 1.1.0 exposition_carrier section），
      用于验证 exposition 提质效果。
    - 生成后由 scripts/run_170g_reeval.py 做 Ch28-Ch40 抽读复评，本脚本只负责生成。
    - 逐章 metrics 追加到 .tmp/task170g_ch1_ch40_metrics.jsonl。
    - 输出路径全部带 task170g_ 前缀，避免覆盖 170b/170f 基线产物。
"""
...
DB_PATH = Path(".tmp/task170g_ch1_ch40.db")
...
METRICS_PATH = Path(".tmp/task170g_ch1_ch40_metrics.jsonl")
...
async def main() -> None:
    ...
    print("下一步: python scripts/run_170g_reeval.py")
