"""Task 170g: Ch28-Ch40 提质后抽读复评（文学提质专项出口判定）.

用法:
    # 在生成脚本跑完 Ch1-Ch40 后执行（复用同一隔离 DB）
    python scripts/run_170g_reeval.py

    # 可指定窗口 / 项目
    $env:ASSESS_START="28"; $env:ASSESS_END="40"
    python scripts/run_170g_reeval.py --project-id <pid>

    # 跳过 LLM 初评（只导出正文 + 机器分 + 洁净度 + exposition 载体，供纯人工实读）
    python scripts/run_170g_reeval.py --no-llm

说明:
    - 助手初筛：抽 Ch28-Ch40 accepted 正文 + LiteraryAuditor 四维机器分 + T9 洁净度
      + exposition 载体硬灌检测 + run_log 关键字段；按 5 维 rubric 做 LLM 初评，
      标出可疑段落与机器/人工偏差候选。
    - 与 170b 基线逐维对比，判定 pass/observation/blocker。
    - 不改生成侧任何 Agent；不改 accept 行为；不放宽任何冻结口径。
    - 正文导出到 .tmp/task170g_prose_ch28_ch40.md（供人工逐章实读）。
    - 报告输出到 docs/reports/task-170g-remediation-reeval-report.md，
      不覆盖 170b/170f 基线报告。
"""
...
settings.database_url = os.getenv(
    "DATABASE_URL", "sqlite:///.tmp/task170g_ch1_ch40.db"
)

PROJECT_FILE = Path(".tmp/task170g_project.json")
REPORT_PATH = Path("docs/reports/task-170g-remediation-reeval-report.md")
PROSE_EXPORT_PATH = Path(".tmp/task170g_prose_ch28_ch40.md")

ASSESS_START = int(os.getenv("ASSESS_START", "28"))
ASSESS_END = int(os.getenv("ASSESS_END", "40"))
...
async def _load_accepted_content(project_id: str) -> dict[int, dict[str, Any]]:
    """按 head 指针取 Ch28-Ch40 的 accepted 正文."""
...
def _export_prose(chapters: dict[int, dict[str, Any]]) -> None:
    ...
    lines: list[str] = [f"# Task 170g 抽读正文 Ch{ASSESS_START}-Ch{ASSESS_END}", ""]
...
def _write_report(
    ...
    # 7. exposition 载体明细
    lines.append("## 7. exposition 载体硬灌明细（Task 170g 代码检测）")
    lines.append("")
    if total_carriers == 0:
    ...
    # 8. 与 170b 基线对比
    lines.append("## 8. 与 170b 基线对比")
    lines.append("")
    lines.append("| 维度 | 170b 基线 | 170f Stage 2 | 170g 目标 | 备注 |")
    lines.append("|------|:---:|:---:|:---:|------|")
    lines.append("| voice | 1.8 | - | 可测量提升 | 需 LLM rubric / 机器 character_autonomy 双重验证 |")
    lines.append("| pacing | 2.4 | 3.25 ✅ | 保持 ≥3.0 | 170f 已验证 scene_interaction 有效 |")
    lines.append("| exposition | 2.1 | 2.0 ❌ | ≥2.5 | 170g 核心攻坚指标 |")
    lines.append("| concept | 3.2 | 3.0 | 不回退 | |")
    lines.append("| ai_tone | 2.2 | 2.0 | 不回退 | |")
    lines.append("| T9 近似重复 | 漏报 | 0 | 不漏报 | 170c 已补强 |")
    lines.append("")
    lines.append(
        "> 170g 判定：需综合 LLM rubric 均值、机器分、exposition 载体检测、"
        "T9 硬红线、用户复核终评分后给出 pass/observation/blocker。"
    )
    lines.append("")

    # 9. 初筛结论（助手观察，非最终判定）
    lines.append("## 9. 初筛观察（助手，非最终判定）")
    lines.append("")
    means = [
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Task 170g Ch28-Ch40 提质后复评")
