"""Task 170h: 中段窗口 Ch1-Ch40 提质后重生成（文学提质专项出口验证）.

用法:
    # 1. 初始化干净隔离 DB + 创建带大纲项目（复用 157b/170b 科幻大纲种子）
    python scripts/run_170h_generation.py --init

    # 2. 真实 LLM 跑 Ch1-Ch40（enforce 门禁，on_failure=isolate）
    python scripts/run_170h_generation.py

    # 中途 kill / AutoHalt 后续跑（复用同一 run_id，跳过已 accepted 章）
    python scripts/run_170h_generation.py --resume

说明:
    - 复用 157b/170b 的脚手架与科幻大纲种子（轨道蜃景 / 林渊 / 方舟-共鸣-旧日搭档三主线），
      保证与 170b 基线同种子、可比。
    - 本脚本加载 Task 170h 新增说明文载体约束（Writer 1.2.1 + CreativeDirector 1.0.7 scene_templates + non-character voice quota section），
      用于验证 exposition 提质效果。
    - 生成后由 scripts/run_170h_reeval.py 做 Ch28-Ch40 抽读复评，本脚本只负责生成。
    - 逐章 metrics 追加到 .tmp/task170h_ch1_ch40_metrics.jsonl。
    - 输出路径全部带 task170h_ 前缀，避免覆盖 170b/170f 基线产物。
"""
...
DB_PATH = Path(".tmp/task170h_ch1_ch40.db")
...
METRICS_PATH = Path(".tmp/task170h_ch1_ch40_metrics.jsonl")
PROJECT_FILE = Path(".tmp/task170h_project.json")

START_CHAPTER = int(os.getenv("START_CHAPTER", "1"))
END_CHAPTER = int(os.getenv("END_CHAPTER", "32"))


# --------------------------------------------------------------------------- #
# 项目设定 + 叙事骨架（复用 157b/170b 种子，便于与历史窗口对比）
# --------------------------------------------------------------------------- #
...
async def main() -> None:
    ...
    print("下一步: python scripts/run_170h_reeval.py")
