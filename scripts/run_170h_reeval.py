"""Task 170h: Ch29-Ch32 提质后抽读复评（文学提质专项出口判定）.

用法:
    # 在生成脚本跑完 Ch1-Ch40 后执行（复用同一隔离 DB）
    python scripts/run_170h_reeval.py

    # 可指定窗口 / 项目
    $env:ASSESS_START="28"; $env:ASSESS_END="40"
    python scripts/run_170h_reeval.py --project-id <pid>

    # 跳过 LLM 初评（只导出正文 + 机器分 + 洁净度 + exposition 载体，供纯人工实读）
    python scripts/run_170h_reeval.py --no-llm
...
说明:
    ...
    - 正文导出到 .tmp/task170h_prose_ch28_ch40.md（供人工逐章实读）。
    - 报告输出到 docs/reports/task-170h-remediation-reeval-report.md，
      不覆盖 170b/170f 基线报告。
"""
...
settings.database_url = os.getenv(
    "DATABASE_URL", "sqlite:///.tmp/task170h_ch1_ch40.db"
)

PROJECT_FILE = Path(".tmp/task170h_project.json")
REPORT_PATH = Path("docs/reports/task-170h-remediation-reeval-report.md")
PROSE_EXPORT_PATH = Path(".tmp/task170h_prose_ch28_ch40.md")
...
def _export_prose(chapters: dict[int, dict[str, Any]]) -> None:
    ...
    lines: list[str] = [f"# Task 170h 抽读正文 Ch{ASSESS_START}-Ch{ASSESS_END}", ""]
...
def _write_report(
    ...
    # 7. exposition 载体明细
    lines.append("## 7. exposition 载体硬灌明细（Task 170h 代码检测）")
    lines.append("")
    if total_carriers == 0:
    ...
    lines.append("")
    lines.append("| 维度 | 170b 基线 | 170f Stage 2 | 170h 目标 | 备注 |")
    lines.append("|------|:---:|:---:|:---:|------|")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Task 170h Ch29-Ch32 提质后复评")
