"""Task 170i: 中段窗口 Ch1-Ch32 重生成（文学提质专项 170i 验证）.

用法:
    # 1. 初始化干净隔离 DB + 创建带大纲项目
    python scripts/run_170i_generation.py --init

    # 2. 真实 LLM 跑 Ch1-Ch32（enforce 门禁，on_failure=isolate）
    python scripts/run_170i_generation.py

    # 中途 kill / AutoHalt 后续跑
    python scripts/run_170i_generation.py --resume

说明:
    - 复用 157b/170b/170h 的脚手架与科幻大纲种子，保证与历史窗口可比。
    - 本脚本加载 Task 170i 新增约束：
      Writer 1.2.2（人类角色声纹锚定 + 认知冲突五节拍 + 主角连续内心独白限制）
      CreativeDirector 1.0.8（认知冲突-误判-代价模板 + 人类角色声纹锚定）
      RevisionHandler 1.1.2（认知冲突缺失 → 人类声纹同质化 → 主角总结容器优先级 patch）
    - 生成后由 scripts/run_170i_reeval.py 做 Ch29-Ch32 抽读复评。
    - 逐章 metrics 追加到 .tmp/task170i_ch1_ch32_metrics.jsonl。
"""
...
DB_PATH = Path(".tmp/task170i_ch1_ch32.db")
...
METRICS_PATH = Path(".tmp/task170i_ch1_ch32_metrics.jsonl")
...
async def main() -> None:
    ...
    print("下一步: python scripts/run_170i_reeval.py")
