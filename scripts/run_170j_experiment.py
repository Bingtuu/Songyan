"""Task 170j: Ch29-Ch32 minimal_voice_anchor experiment harness.
...
说明:
    ...
    - 启用 Task 170j 的 minimal_voice_anchor 文学优化策略。
    - 默认 GATE_MODE=observe，便于小样本快速验证。
    - 生成后由 scripts/run_170j_reeval.py 做 Ch29-Ch32 抽读复评。
    - 逐章 metrics 追加到 .tmp/task170j_minimal_voice_anchor_metrics.jsonl。
"""
...
def _resolve_project_file(strategy_id: str) -> Path:
    return Path(f".tmp/task170j_{strategy_id}_project.json")


def _resolve_metrics_path(strategy_id: str) -> Path:
...
PROJECT_FILE = _resolve_project_file(DEFAULT_STRATEGY_ID)
METRICS_PATH = _resolve_metrics_path(DEFAULT_STRATEGY_ID)
...
async def _init_experiment(strategy_id: str) -> str:
    ...
    project_file = _resolve_project_file(strategy_id)
...
def _resolve_project_id(strategy_id: str) -> str | None:
...
def _append_metric(record: dict[str, Any], strategy_id: str) -> None:
    metrics_path = _resolve_metrics_path(strategy_id)
...
async def main() -> int:
    ...
    # 根据策略 ID 重新定向 DB / project / metrics 路径
    db_path = _resolve_db_path(args.strategy_id)
    ...
    project_id = args.project_id or _resolve_project_id(args.strategy_id)
    if not project_id:
    ...
    print(f"[preflight] strategy={args.strategy_id}")
    ...
    for ch in sorted(run_log.keys()):
        if args.start <= ch <= args.end:
            _append_metric({"chapter": ch, **run_log[ch]}, args.strategy_id)
    ...
    print(f"Metrics: {_resolve_metrics_path(args.strategy_id)}")
    print("下一步: python scripts/run_170j_reeval.py")
    return 0
