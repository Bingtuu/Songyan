"""Task 170k: Ch29-Ch32 opposing_goal_anchor experiment harness.

用法:
    # 1. 初始化干净隔离 DB + 创建带大纲项目
    python scripts/run_170k_experiment.py --init

    # 2. 真实 LLM 跑 Ch29-Ch32（默认 observe 门禁，on_failure=isolate）
    python scripts/run_170k_experiment.py --start 29 --end 32

说明:
    - 复用 Task 170i 的种子大纲与项目设定，保证窗口可比。
    - 叠加启用 Task 170j 的 minimal_voice_anchor 与 Task 170k 的 opposing_goal_anchor。
    - 默认 GATE_MODE=observe，便于小样本快速验证。
    - 生成后由 scripts/run_170k_reeval.py 做 Ch29-Ch32 抽读复评。
    - 逐章 metrics 追加到 .tmp/task170k_opposing_goal_anchor_metrics.jsonl。
"""
...
# 170k 同时启用 170j 的 minimal_voice_anchor + 本任务新增的 opposing_goal_anchor
DEFAULT_STRATEGY_SLUG = "opposing_goal_anchor"
DEFAULT_STRATEGY_IDS = ["minimal_voice_anchor", "opposing_goal_anchor"]
DEFAULT_START_CHAPTER = 29
DEFAULT_END_CHAPTER = 32


def _resolve_db_path(strategy_slug: str) -> Path:
    return Path(f".tmp/task170k_{strategy_slug}.db")


def _resolve_project_file(strategy_slug: str) -> Path:
    return Path(f".tmp/task170k_{strategy_slug}_project.json")


def _resolve_metrics_path(strategy_slug: str) -> Path:
    return Path(f".tmp/task170k_{strategy_slug}_metrics.jsonl")
...
def _ensure_temp_mode_profile(strategy_slug: str, strategy_ids: list[str]) -> str:
    ...
    target_path = modes_dir / f"webnovel_intense_{strategy_slug}.json"
...
async def main() -> int:
    ...
    print("下一步: python scripts/run_170k_reeval.py")
    return 0
