"""Task 170k: Ch29-Ch32 opposing_goal_anchor 抽读复评.

用法:
    # 在生成脚本跑完 Ch29-Ch32 后执行
    python scripts/run_170k_reeval.py

    # 可指定窗口 / 项目
    $env:ASSESS_START="28"; $env:ASSESS_END="32"
    python scripts/run_170k_reeval.py --project-id <pid>

    # 跳过 LLM 初评（只导出正文 + 机器分 + 洁净度 + exposition 载体）
    python scripts/run_170k_reeval.py --no-llm

说明:
    - 抽 Ch29-Ch32 accepted 正文 + LiteraryAuditor 四维机器分 + T9 洁净度
      + exposition 载体硬灌检测 + run_log 关键字段；按 5 维 rubric 做 LLM 初评。
    - 与 170j 基线逐维对比，判定是否达到 Ch200 入口标准。
    - 正文导出到 .tmp/task170k_prose_ch28_ch32.md。
    - 报告输出到 docs/reports/task-170k-opposing-goal-anchor-reeval-report.md。
"""
...
settings.database_url = os.getenv(
    "DATABASE_URL", "sqlite:///.tmp/task170k_opposing_goal_anchor.db"
)

PROJECT_FILE = Path(".tmp/task170k_opposing_goal_anchor_project.json")
REPORT_PATH = Path("docs/reports/task-170k-opposing-goal-anchor-reeval-report.md")
PROSE_EXPORT_PATH = Path(".tmp/task170k_prose_ch28_ch32.md")
...
def _export_prose(chapters: dict[int, dict[str, Any]]) -> None:
    ...
    lines: list[str] = [f"# Task 170k 抽读正文 Ch{ASSESS_START}-Ch{ASSESS_END}", ""]
...
def _write_report(
    ...
    lines.append("## 7. exposition 载体硬灌明细（Task 170k 代码检测）")
    lines.append("")
    if total_carriers == 0:
    ...
    lines.append("## 8. 与 170j 基线对比")
    lines.append("")
    lines.append("| 维度 | 170j 基线 | 170k 目标 | 备注 |")
    lines.append("|------|:---:|:---:|------|")
    lines.append("| voice | 2.25 | ≥3.0 | 170k 核心攻坚指标：对抗性目标锚定 |")
    lines.append("| exposition | 2.25 | ≥3.0 | 保持/提升，不回退 |")
    lines.append("| pacing | 3.25 | ≥3.0 | 保持，不回退 |")
    lines.append("| concept | 3.25 | ≥3.0 | 保持，不回退 |")
    lines.append("| ai_tone | 2.00 | ≥3.0 | 保持，不回退 |")
    ...
    if window_mean is not None:
        if window_mean >= 3.0 and total_carriers <= 1 and total_meta == 0 and total_dup == 0:
            ...
            lines.append(
                ...
                "exposition_carrier≤1），维持 blocker，继续 170k+ 迭代或升级路径。"
            )
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Task 170k Ch29-Ch32 opposing_goal_anchor 复评")
    ...
    if not project_id:
        parser.error(
            ...
            "（写 .tmp/task170k_opposing_goal_anchor_project.json）"
        )
