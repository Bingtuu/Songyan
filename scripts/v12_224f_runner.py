"""V12 Task 224f runner: clean seed-aligned rebuild + plan-only + review-plan for Ch1.
...
Usage:
    python scripts/v12_224f_runner.py
"""
...
def clone_seed(source_project_id: str, new_project_id: str) -> str:
    ...
    try:
        ...
        con.commit()
        print(f"[224f] cloned seed: {source_project_id} -> {new_project_id}")
        print(f"[224f] new character_id: {new_character_id} (characters={len(crows)})")
...
async def step_plan_only(project_id: str):
    print(f"\n[224f] === plan-only for Ch1 (project={project_id}) ===")
...
async def step_review(plan_result):
    print("\n[224f] === deterministic review-plan ===")
...
def step_save_artifacts(plan_result, review, stamp: str) -> Path | None:
    run_dir = RUN_DIR / f"v12_task224f_ch1_plan_{stamp}"
...
async def main() -> int:
    stamp = gen_suffix()
    project_id = f"hard-sf-new-weird-v12-224f-{stamp}"

    print(f"[224f] V12 Task 224f clean rebuild + Ch1-only closure - started at {stamp}")
    print(f"[224f] source seed project: {SOURCE_PROJECT_ID}")
    print(f"[224f] new project_id     : {project_id}")
    print(f"[224f] supervision spec   : {SPEC_PATH}")
    ...
    if not review.passed:
        print("\n[224f] STOP: review-plan rejected. Report findings to user; do NOT auto-retry.")
        return 2

    print("\n" + "=" * 60)
    print("[224f] Plan review PASSED. Now run REAL SMOKE with:")
    print("=" * 60)
    print(f"""
...
songyan export --project-id {project_id} --chapters 1 --format md --output projects/hard-sf-new-weird/exports/ch001_round_v12_224f/
