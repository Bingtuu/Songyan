...
- [ ] 生成 `tasks/164-text-cleanliness-metrics-DONE.md`（含度量入库设计、report 段样例、T9 harness 接口、参数化说明）。
...
- 判据 harness：`src/songyan/evals/v6_acceptance.py`（`evaluate_v6_acceptance` / `ThresholdResult` / `render_v6_acceptance_section`）、Task 157 `tasks/157-ch1-ch50-integration-validation-DONE.md`
- 上游检测源：Task 160/161/162 的 `detect_markdown_scene_titles` / `detect_duplicate_paragraphs` / `detect_timeline_conflicts`
- 阈值冻结纪律：`tasks/148z-stage-a-threshold-calibration-DONE.md`（148z 先实测再冻结）
