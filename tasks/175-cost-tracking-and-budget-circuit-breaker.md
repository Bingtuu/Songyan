...
- **拦截点理想**：全部 12 个 Agent LLM 调用点（writer / llm_auditor / literary_auditor / revision_handler×4 / settlement_extractor / summary_writer / arc_summary_generator×2 / creative_director×2 / goal_planner）都经 `call_llm()`（`llm/client.py:154`）单一漏斗；唯一的 `ainvoke` 直调用是 phase1_graph 的 LangGraph graph 调用，非 LLM 调用。
...
1. 每次 LLM 调用（含每次重试尝试）落一行 `llm_call_usage`：run/chapter/agent/stage/model/tokens/cost/latency/retry_attempt/success/error，usage 来源标记（response 精确值 / estimate 估算）。
2. run 级成本预算 `run_cost_budget`（默认 0=不限）：累计超限抛 `LLMBudgetExceededError`，走既有优雅停跑路径（pause + 保留已生成章 + 可 resume）；与 `llm_run_call_budget` 调用次数预算相互独立。
3. `ProjectRunResult.total_cost` 接真实值；`songyan report` 含成本视图（per run / per chapter / per agent）。
...
**1. 新表 `_migrate_llm_call_usage`**（`db/migrations.py`，仿 `_migrate_run_db_metrics` 模式，两个 init 路径都注册）：
...
CREATE TABLE IF NOT EXISTS llm_call_usage (
    ...
    usage_source TEXT NOT NULL, -- 'response' | 'estimate'
...
- `_invoke()` 内（`client.py:200-218`）按尝试记录：成功时先取 `response.usage_metadata`（langchain-core 标准：`input_tokens/output_tokens`），缺失取 `response.response_metadata` 的 token_usage（litellm 风格），仍缺失回退 `cost_estimator.estimate_cost(prompt, text, model)` 并标 `usage_source='estimate'`；失败尝试记 `success=0` + `error` + 零 usage。`latency_ms` 用 `time.monotonic()` 夹取。`retry_attempt` 从 `retry_with_backoff` 的尝试序号透传（需给 retry helper 加一个可选 attempt 回调参数，或把记录点放在 `_invoke` 内部闭包计数）。
- **agent 归因**：12 个 Agent 调用点函数入口各加一行 `bind_contextvars(agent="<name>")`（覆盖语义，下一个 Agent 入口自然覆盖前值，无泄漏窗口）；调用点：writer、llm_auditor、literary_auditor、revision_handler（`__init__`×4 与 `_segmented_revision`）、settlement_extractor、summary_writer、arc_summary_generator、creative_director、goal_planner。

### 阶段 B：run 级累计与熔断

**1. 累计器**：`client.py` 新增 `_llm_run_cost_cny: ContextVar[float]`（镜像既有 `_llm_call_count` 模式）；每次成功记录后累加。
...
**3. 熔断**：`call_llm` 前置检查（紧挨既有调用次数预算检查 `client.py:185-196`）：
...
**4. `total_cost` 接线**：`phase2_graph.py:1060` 替换为 `await LLMCallUsageRepository().sum_cost_for_run(run_id)`（run_id 为 None 时 0.0）。

### 阶段 C：report 成本视图

`cli/main.py:583` `report_cmd` 增加成本段（数据源 `aggregate_for_run`）：run 总成本（¥）、章节数与每章均成本、per agent 成本分布（top N）、`usage_source='estimate'` 占比（估算占比高 = usage 提取需要修的早期信号）。无 usage 数据的旧 run 显示"无成本数据"不报错。
...
- response 带 `usage_metadata` → 落库 `usage_source='response'`、tokens/cost 精确；
- response 无 usage → estimate fallback，`usage_source='estimate'`；
...
- 熔断：预算 ¥0.01 + 预置累计 → `LLMBudgetExceededError`，`used_cost/budget_cost` 字段正确；
- resume 初始化：DB 预置历史行 → `init_run_cost_from_db` 后累计器从历史值继续；
- **repo.record 抛异常时 `call_llm` 正常返回**（telemetry 不阻断生成）；
- `total_cost` 接线：构造 run 用量行后 `ProjectRunResult.total_cost` 等于 DB 合计（或拆为 sum 查询单测 + 接线点断言）；
...
- scifi `--end 10` 10/10、Ch1 budget=8250（旧行为逐值不变）；`llm_call_usage` 落库完整、estimate 占比 <20%；report 成本视图可用；
...
1. `llm_call_usage` 表 + repository + 12 个调用点 agent 归因落地，全量测试绿；
