...
> **完成时间**: 2026-07-19
> **阶段**: V9.1 长跑可靠性
> **状态**: ✅ 完成（wrapper 落地 + 自检 9/9 + 双 review 修复 + 实跑验收通过）
> **任务书**: `tasks/176-windows-anti-hang-wrapper.md`（含执行记录）
...
- **`-SelfTest` 自检模式**：9 项矩阵自动化并落盘证据（成功/失败/超时杀树复核/防误判/teardown 宽限/参数转义/pytest 子集/business marker/auto-enable 端到端）。
...
| 命令 / 证据 | 结果 |
|---|---|
| `powershell -File scripts/run_with_timeout.ps1 -SelfTest` | **9/9 ALL_PASS**（PS 5.1.26100；真实杀树复核、误判防护、参数转义、business marker、auto-enable 全覆盖），证据 `.tmp/176_selftest/` |
...
- Review 修复记录（4 Important + 4 Minor）与参数接收偏离的完整论证见任务书执行记录。
