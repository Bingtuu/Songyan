...
python scripts/<ch200_harness>.py --audit --genre <genre> --up-to 150
...
| 项 | 要求 |
|----|------|
| source DB | 只能来自 Task 190 判定为 `CONTINUE_READY` 的 clean Ch100 DB |
...
| source inventory | 写入或引用 `.tmp/190_ch100_source_inventory.json` 中对应体裁记录 |
...
9. 有聚焦测试或脚本 dry-run 证据；`git diff --check` 通过。
10. 产出 `tasks/191-ch200-harness-preparation-DONE.md`，并更新 `tasks/V10-README.md`。
...
| baseline 文件格式未冻结 | 依赖 Task 189；本任务只留接口 |
