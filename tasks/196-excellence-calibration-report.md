...
> **完成时间**: 2026-07-29
> **数据**: `tasks/196-excellence-sample-set.json`（60 章样本，seed=196）、`tasks/196-excellence-annotations.json`（72 条标注）、`.tmp/196_rule_pilot.json`（规则试点）、`.tmp/196_sample_texts/`（60 章正文快照，复算用）
...
| 产物 | 路径 | 内容 |
|------|------|------|
| 样本清单 | `tasks/196-excellence-sample-set.json` | 60 章：xuanhuan 冻结库（`.tmp/task_v10_xuanhuan_ch200.db`，project `d160a55a…`）+ sci-fi 冻结库（`.tmp/task171_ch1_ch200.db`，project `835afdf1…`）各 30 章；seed=196，segment_size=25，8 个弧段每段 3-4 章全覆盖；每样本含 genre/chapter/version_id/segment |
| 标注记录 | `tasks/196-excellence-annotations.json` | 72 条 AnnotationRecord：anchor 12 + prelabel 48 + spotcheck 12（实测分层计数 48/12/12） |
...
- **样本清单**：`tasks/196-excellence-sample-set.json`（seed=196，可复现；60 章正文快照 `.tmp/196_sample_texts/` 不版本管理，可用 `scripts/build_196_sample_set.py` 从冻结库重建）。
- **标注记录**：`tasks/196-excellence-annotations.json`，schema 见 §1；消费前按 `sample_layer` / `annotator` 过滤。
- **各下游任务消费层**：
