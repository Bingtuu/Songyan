...
| Task | 名称 | 状态 | 内容要点 | 验收要点 |
|------|------|:----:|----------|----------|
| 192 | xuanhuan Ch200 爬坡 | ◐ | clean Ch100 source ready；Task 191 harness 已初始化 Ch200 target；Ch125 checkpoint PASS；Ch150 checkpoint PASS；Ch156 clean；Ch162 clean；Ch168 clean；Ch175 clean；Ch180 clean；Ch186 clean；Ch192 clean；Ch198 accepted 后触发 `health_low_p1_halt`，已冻结并建立 192.av；下一步先修复 192.av，不得继续 Ch199/200 | 任务书：`tasks/192-xuanhuan-ch200-climb.md`；执行报告：`docs/reports/192-xuanhuan-ch100-climb.md` |
...
| 192.av | xuanhuan Ch198 health_low_p1_halt | ◐ | Ch193-Ch198 accepted；Ch198 accepted/current head `v-72d5a104` 后触发 `health_low_p1_halt: P1_count=1`；P1 target=`xuanhuan_lingyuan_technique.escape.shadow_step`；run paused，completed 1..198，failed=[]；现场已冻结 | 任务书：`tasks/192.av-xuanhuan-ch198-health-low-p1-halt.md` |
