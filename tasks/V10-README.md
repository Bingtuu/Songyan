...
| Task | 名称 | 状态 | 内容要点 | 验收要点 |
|------|------|:----:|----------|----------|
| 192 | xuanhuan Ch200 爬坡 | ◐ | clean Ch100 source ready；Task 191 harness 已初始化 Ch200 target；Ch125 checkpoint PASS；Ch150 checkpoint PASS；Ch151-Ch156 accepted 后触发 192.ao `health_low_streak_halt`；修复前不得继续 Ch157/175 | 任务书：`tasks/192-xuanhuan-ch200-climb.md`；执行报告：`docs/reports/192-xuanhuan-ch100-climb.md` |
...
| 192.ao | xuanhuan Ch156 health_low_streak_halt | ◐ | Ch151-Ch156 accepted；Ch156 continuity audit health=7.7，unresolved P2 foreshadowing=10，触发 `health_low_streak_halt: window=3 P2_total=10 >= limit=2`；run paused、failed=[] | 任务书：`tasks/192.ao-xuanhuan-ch156-health-low-streak-halt.md` |
