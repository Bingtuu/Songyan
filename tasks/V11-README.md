...
Task 209 留给后续实现的 runtime 缺口中，非法 `CHECKPOINTER_MODE` traceback 与 `run` 业务失败 exit code 语义已由 Task 210 收口；backup/restore 已由 Task 211 收口；失败恢复分类和提示已由 Task 212 收口。
...
Task 210 留给后续实现的项目资产缺口已由 Task 211 收口。
...
## Task 211 backup / restore 结论

Task 211 已完成。任务书见 `tasks/211-v11-backup-restore-schema-ledger.md`，DONE 见 `tasks/211-v11-backup-restore-schema-ledger-DONE.md`，命令证据见 `docs/reports/211-backup-restore-evidence.md`。

交付：

- 新增 `songyan backup --project-id <project_id> --output <dir-or-zip>`，生成项目资产 zip。
- 新增 `songyan restore --backup <zip> --database-url sqlite:///restored.db [--force]`。
- 资产包包含 SQLite 一致性快照、manifest、schema ledger、项目配置摘要、运行摘要和关键日志索引。
- restore 默认拒绝覆盖已有 DB，显式 `--force` 才允许覆盖。
- 资产包默认不包含 `.env` 原文、API key 或日志正文。
- 文档明确 `export` 只导出 accepted 正文，`backup` 保存可恢复 / 可迁移资产。
...
Task 212 已完成。任务书见 `tasks/212-v11-failure-recovery.md`，DONE 见 `tasks/212-v11-failure-recovery-DONE.md`，命令证据见 `docs/reports/212-failure-recovery-evidence.md`。

交付：

- 新增统一恢复分类：`config_error`、`database_error`、`preflight_failed`、`run_failed`、`missing_artifact`、`no_accepted_content`、`asset_restore_error`。
- `songyan doctor` human 输出在失败时附恢复建议。
- `songyan run` preflight fail 输出分类和修复命令。
- pipeline 已启动后失败会保留 `run_id` 并提示 `songyan report --run-id <run_id>`。
- `songyan report` 缺 JSONL 时 exit 1，并提示检查 `logs/chapter_runs` 和 run_id。
- `songyan export` 无 accepted 章节时提示先生成 accepted 章节。
- `songyan backup` / `restore` 资产问题输出可执行恢复命令。
