...
> **阶段**: 篇章级质量修复 → 叙事自驱 → enforce 可生产化 → **sci-fi 单一体裁 Ch200 达成收尾**
> **收尾时间**: 2026-07-14
> **收尾判定**: V7 原定的 Ch250/Ch300 单一体裁爬坡不再继续；sci-fi/space_opera + webnovel_intense 已在 Ch200 达成 200/200 accepted + D1 hard clean pass，171w hardening 与 Ch201-Ch220 20/20 accepted 已落地。多体裁泛化与原 Task 172/173 单一体裁目标已迁移至 **V8**，见 `tasks/V8-README.md`。
> **当前口径**: **V7 阶段 W/X/Y/Z（至 Ch200）已通过，T9/T10/T12 已冻结；文学提质专项（Task 170）已结束——改判为"改契约 + 并行"框架**。旧框架把文学质量（voice/exposition ≥3.0）设为 Ch200 硬前置门，经框架级复盘认定其有 5 个结构性错误，路径 B 五步 prompt 工程（170h–170l）在错误框架内必然递减/劣化，**已封存**。新框架把文学质量拆为三层契约：Tier 1 硬缺陷（T9，仍阻塞）+ Tier 2 趋势地板（observe 不阻塞）+ Tier 3 上限（并行 R&D）。**Task 171 Ch200 长跑已取得 `run-fb39245c` 200/200 accepted + D1 hard clean pass 证据**；171w hardening 已完成，Ch201-Ch220 20/20 accepted、failed=[]、Halt=None。
> **最后整理**: 2026-07-14（V7 收尾；172 取消归档；173 迁移至 V8）

本文是 V7 阶段任务文档的历史事实入口。当前阶段入口见 `tasks/V8-README.md`；V6 阶段事实入口见 `tasks/V6-README.md`；V5 见 `tasks/V5-README.md`；历史规划稿统一归档到 `archive/`，仅在追溯设计边界时查阅。V7 各任务最终状态以本文件和各 `*-DONE.md` 为准。

---

## 一句话目标

> **V7 让系统"自己把质量维持在高位"——先修复 V6 暴露的篇章级质量债（文本洁净、去重、概念落地），再闭合文学质量、伏笔调度、enforce 门禁三个开放环，最终在 sci-fi 单一体裁下验证到 Ch200。文学质量环（Task 170）经框架级复盘，从"voice/exposition ≥3.0 的 Ch200 硬前置门"改判为"三层契约 + 并行 R&D"（详见 `docs/reports/v7-literary-framework-review.md`）：Tier 1 硬缺陷阻塞、Tier 2 趋势地板转 observe 不阻塞、Tier 3 上限归并行 R&D。Task 171 Ch200 长跑已取得 200/200 accepted + D1 hard clean pass 证据；171w hardening 已完成，Ch201-Ch220 20/20 accepted。本阶段目标不再是"把 voice 拧到 3.0"，而是"建立可信、可证伪、体裁解耦、不阻塞规模化的文学质量框架，并取得 Ch200 真实证据"（阶段验收标准见框架文档 §8）。**

四个决策边界（2026-07-04 确认，详见 `archive/v7/plans/v7-plan.md` §1.2）：
1. **质量修复优先**：先修 159 暴露的篇章级缺陷，再做长程爬坡。
2. **文学修复保守**：以确定性工程修复（清洗元标记、段落去重、概念预算）+ 诊断告警 + 人工介入点为主；**不做全自动 LLM 改写闭环**。
3. **渐进爬坡 Ch200**：在单一体裁内取得真实证据，不再继续 Ch250/Ch300 产品化爬坡。
4. **不纳入题材泛化（产品化）**：专注科幻单题材把质量做到 Ch200；genre 配置化 + 非科幻**产品化验证**划归 V8。

---

## 阶段验收判定（P/L/T/G/V）

V7 通过 = 同时满足以下五项（阈值沿用 v6-plan §1.4 的 T1-T8，V7 新增 T9-T12；T9/T10 已由阶段 W 用 Ch150 修复后基线冻结、T12 已由阶段 Y 的 Task 170 小窗口验证冻结）：
...
| **V（验证）** | 取得 Ch200 真实证据（run `run-fb39245c`），200/200 accepted；事实源质量不随长度衰减 |

> **文学维度口径变更（2026-07-10）**：L 维度原表述"全程无文学维度触 T3 红线"已按框架级复盘改判为三层契约——文学质量不再作 Ch200 的硬阻塞门，改为 Tier 1 硬缺陷阻塞 + Tier 2 趋势观测 + Tier 3 并行 R&D。本阶段（含文学环）的完整 PASS 判据以 `docs/reports/v7-literary-framework-review.md` §8（A/B/C/D/E 五组）为准。

---

## Task 状态

> 状态口径：`◻ 规划中`（有规划稿，未开工）/ `🔄 进行中` / `✅ 完成`（有 `*-DONE.md`）/ `⚠️ 条件完成` / `⚠️ 条件未通过` / `❌ 已取消/归档` / `⏳ 占位`（骨架占位，详细文档待前置数据出炉后写）。
...
| 170h | 路径 B 结构性改写：场景模板约束 + 非人实体戏份分配 + 声纹工程升级 | 提质 | ✅ 完成（维持 blocker） | `archive/v7/tasks/170h-structural-rewrite-voice-exposition.md`（规划）、`archive/v7/tasks/170h-structural-rewrite-voice-exposition-DONE.md`（DONE）、`archive/v7/reports/task-170h-remediation-reeval-report.md` |
| **170i** | **路径 B 第二步：主角认知冲突/误判代价 + 人类角色声纹锚定** | **提质** | **✅ 完成（维持 blocker）** | **`archive/v7/tasks/170i-protagonist-cognitive-conflict-voice-anchoring.md`**、**`archive/v7/tasks/170i-protagonist-cognitive-conflict-voice-anchoring-DONE.md`**、**`archive/v7/reports/task-170i-remediation-reeval-report.md`** |
| **170i-patch** | **170i 量具补丁：修复 `detect_human_voice_homogeneity` 恒为 0 假阴性** | **量具** | **✅ 完成（2026-07-10）** | 补丁记录见 `archive/v7/tasks/170i-protagonist-cognitive-conflict-voice-anchoring-DONE.md` §量具补丁 |
| **170j** | **路径 B 第三步：最小声纹锚定（minimal_voice_anchor）** | **提质** | **✅ 完成（维持 blocker）** | **`archive/v7/tasks/170j-ai-tone-voice-feasibility-assessment-DONE.md`** |
| **170k** | **路径 B 第四步：角色对抗性目标锚定（opposing_goal_anchor）** | **提质** | **✅ 完成（维持 blocker）** | **`archive/v7/tasks/170k-opposing-goal-anchor-DONE.md`**、**`archive/v7/reports/task-170k-opposing-goal-anchor-reeval-report.md`** |
| **170l** | **路径 B 第五步：声纹工程升级接口化（few_shot_voice_anchor + AI 腔禁用表）** | **提质** | **✅ 完成（维持 blocker）** | **`archive/v7/tasks/170l-few-shot-voice-anchor.md`**、**`archive/v7/tasks/170l-few-shot-voice-anchor-DONE.md`**、**`archive/v7/reports/task-170l-few-shot-voice-anchor-reeval-report.md`** |
...
> **主线**（Ch200 长跑 + 171t/171u/171v/171w 收口）与 **R&D 线**（171a→171b→171c 文学量具/样本/杠杆）并行。R&D 线是"文学结论"的前提，但**不阻塞主线**：Ch200 以已验证稳定性面放行、文学=观测。Ch200 完成后的收口链路为 **171t 量具补强 → 171u 清洁应用/报告复算 → 171v 文学护栏 → 171w hardening + Ch201-Ch220 重验**。171w 已完成，V7 在此收尾，不再继续 Ch250/Ch300。
...
| 172 | Ch250 过渡验证 | ❌ **已取消/归档** | 原 `archive/v7/tasks/172-ch250-transition-validation-archived.md` 已归档至 `archive/v7/tasks/172-ch250-transition-validation-archived.md`；V7 不再继续单一体裁 Ch250 爬坡 |
| 173 | Ch300 终态验收 + V7 阶段验收报告 | ➡ **已迁移至 V8** | 见 `tasks/V8-README.md`；原占位目标取消，多体裁泛化作为 V8 主线 |
...
```
W（篇章级质量修复 160-165p）─────────────► Z（171 Ch200 → 171t/171u/171v → hardening/重验 → V7 收尾）
X（叙事自驱 166-167）──────────┐              │
...
- **阶段 Z 主线与 R&D 线并行**：171/172/173 长跑是终检，V7 在 Ch200 + 171w 收口后收尾，不再继续 Ch250/Ch300。
- **文档递进纪律**：166/166a/166b、167、168、169、170、170b–170p 已落地并有 evidence；文学专项 Task 170 已结束并改判（框架文档 `docs/reports/v7-literary-framework-review.md`）。阶段 Z 的 171 主线、171t/171u/171v 收口保留在当前入口；171a/171a-1/171b/171c/171d R&D 产物已归档至 `archive/v7/`；172 已归档；173 已迁移至 V8。
...
- **2026-07-12 拆分修正**：Ch200 20% 抽读证明 D1 hard clean 问题不只是 duplicate/stale report，而是 T9 artifact false negative；因此把原 171t/171u 拆为 **171t 量具补强、171u 清洁应用/报告复算、171v 文学护栏**。
- **2026-07-14 阶段收尾**：原 172 Ch250 与 173 Ch300 目标取消，归档/迁移至 V8。
- **文档策略**：v7-plan 为全局骨架；阶段 W（160-165p）、阶段 X 的 166/167、阶段 Y 的 168/169/170 已落地详细文档；**文学专项 170b–170p DONE 已补齐并结束**（改判见框架文档）；阶段 Z 的 171 主线、171t/171u/171v 收口 task 保留在当前入口；171a/b/c/d R&D 证据见 `archive/v7/INDEX.md`；172 已归档；173 已迁移至 V8。
...
| 题材泛化——**产品化**（genre 配置化 + 非科幻爬坡验证） | **划归 V8**。注：171a/171b 用 ≥2 体裁做**量具效度交叉验证**是例外，属"证明量具体裁解耦"，非产品化泛化。 |
| Ch250/Ch300 单一体裁继续爬坡 | 已取消，V7 在 Ch200 收尾 |
| 多项目并发 / 分布式长跑 | 不做 |
...
- V7 构想（方向性）：`archive/v7/plans/v7-vision.md`
- V7 阶段规划与 Task 160-173 路线图：`archive/v7/plans/v7-plan.md`
- V6 阶段事实入口（前置）：`tasks/V6-README.md`
- V6 阶段验收报告（篇章级质量债依据）：`archive/v6/reports/task-159-v6-final-acceptance-report.md`
