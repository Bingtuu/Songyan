...
| # | 判据 | 对应 Task |
|---|------|-----------|
| A1 | `pip install .`（非 `-e`）装出的 wheel 在非仓库 cwd 能直接跑通 `create-project --template scifi` + `run --chapters 1-3`（**已闭环**：wheel 安装后 site-packages 资源枚举通过，非仓库 cwd `create-project` + Ch1-3 3/3 accepted，`WRAPPER_RESULT=PASS_NORMAL_EXIT`） | 178 |
...
| A8 | `songyan profile show/diff/upsert --genre <g>` 可用；标定迭代全程不改代码（**已闭环**：registry / DB override / effective 三列视图；upsert 伪稀疏覆盖；reset 清空 override） | 183 |
...
|---|---|---|---|---|
| base_budget / ramp | 8000 / 250 | 15000 / 250 | 10500 / 250 | **14000 / 250（185 初值 12000；187.p Ch19 emergency 后校准）** |
| foreshadowing_horizon_floor | 0 | 48 | 48 | **0（185 确认短窗口无压力；Ch100 再观察）** |
| character_decay.focal_gaps | 3/10/30 | 8/20/60 | 8/20/60 | 3/10/30 |
| continuity.health_overdue_weight | 0.3 | 0.3 | 0.15 | **0.08（187.s Ch25 health 校准）** |
