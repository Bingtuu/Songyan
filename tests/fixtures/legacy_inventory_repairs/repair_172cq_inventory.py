"""Task 172c.q inventory identity repair helper."""
...
def _merge_key(row: dict[str, Any], item_name: str) -> tuple[str, str]:
    ...
    key = norm_base if len(norm_base) >= 2 else _normalize_item_name(item_name)
    return (str(row.get("character_id") or ""), key)


async def repair_inventory(project_id: str, up_to_chapter: int) -> dict[str, int]:
    """Repair legacy wuxia inventory rows by base item identity."""
    ...
    for row in rows:
        ...
        for item_name in item_names:
            ...
            last_used = (
                ...
                else int(row.get("last_used_chapter") or acquired)
            )
            candidate = {
